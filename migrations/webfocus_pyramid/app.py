...
def generate_decode_calculated_field(source_field, derived_field, pairs):
    ...
    for source_value, mapped_value in pairs:
        ...
        args.append(f"'{safe_source}'")
        args.append(f"'{safe_mapped}'")
    args.append("'Unmapped'")
    return f"-- Pyramid Formulate (PQL) Expression: {derived_field}\nCase({', '.join(args)})"
...
def generate_decode_data_source_join_step(source_field, derived_field, lookup_table_name):
    return (
        f"-- Pyramid Data Model setup for {lookup_table_name}\n"
    ...
    for table_name, group in mapping_df.groupby('Mapping Table Name'):
        ...
        if len(pairs) <= switch_threshold:
            packs.append({
                ...
                'recommended_approach': 'Pyramid Formulate PQL calculation (CASE)',
            ...
            packs.append({
                ...
                'derived_field': derived_field,
                'pair_count': len(pairs),
                'recommended_approach': f'Pyramid lookup data source ({lookup_name})',
            ...
            if count >= min_branches:
                results.append({
                    ...
                    'suggestion': (
                        ...
                        'or directly to a Pyramid lookup data source.'
                    ),
                })
    return results
...
def build_rule_findings(parsed, mapping_df, final_table_name):
    ...
    if date_idiom:
        findings.append({
            ...
            'recommendation': 'Convert positional date-string parsing into a typed Date field in the Pyramid data source (Data Prep step or a DATE() calculated field).',
...
def blank_llm_translation(status='Not requested'):
    return {
        'suggested_pyramid_type': '',
        'suggested_pyramid_expression_action': '',
        'llm_notes': '',
...
def translate_expression_with_llm(client, model, expression_type, expression, context):
    ...
    prompt = f"""
You translate WebFOCUS migration logic into Pyramid guidance.
cd 
...
Rules:
- The value for suggested_pyramid_expression_action must be one plain multiline string.
- Do not return suggested_pyramid_expression_action as a JSON array, Python list, bullet list, or numbered list.
...
- Prefer a Pyramid Formulate PQL calculation for row-level DEFINE/COMPUTE logic.
- Prefer a table calculation only when the aggregation/window context is clearly needed (e.g. running total, rank).
- Prefer a Data Prep (Pyramid Data Flow) step or data source join/union for source shaping, filters before load, and cleanup.
...
- Keep data source and field names as placeholders when the exact Pyramid workbook name is unknown.
...
""".strip()
    ...
    try:
        ...
        result = {
            'suggested_pyramid_type': normalize_llm_text(data.get('suggested_pyramid_type', '')),
            'suggested_pyramid_expression_action': normalize_llm_text(data.get('suggested_pyramid_expression_action', '')),
            ...
            'review_reason': normalize_llm_text(data.get('review_reason', '')) or 'Review before using in Pyramid.',
...
AI_BUILD_PLAN_SYSTEM_PROMPT = """You are a senior BI migration engineer converting one parsed WebFOCUS report to Pyramid Analytics.
...
Pyramid Analytics target concepts to use:
- Data Source / Model Table: initial data source, database connection, or physical model table in Pyramid Data Model.
- Data Flow Step: Pyramid Data Flow (Prep) ETL step for staging, joins, column cleanup, or HOLD lineage before model load.
- Semantic Relationship: join relationship defined between tables in the Pyramid Data Model (cardinality, cross-filter).
- Calculated Column: row-level calculated or virtual column in the Pyramid Data Model.
- Formulate Measure (PQL): custom metric, KPI, or aggregate formula created in Pyramid Formulate using PQL (Pyramid Query Language).
- Discover Filter / Slicer: slice, background filter, or interactive slicer configured in Pyramid Discover.
- Parameter / Global List: global parameter or dynamic custom list in Pyramid Formulate (equivalent to WebFOCUS &VARIABLE).
- Discover Visual (Grid/Chart): discovery visual in Pyramid Discover (Grid, Tabular Report, Matrix, Chart).
- Present Dashboard: interactive multi-visual dashboard or storyboard in Pyramid Present.
- Publish Publication: scheduled or burst distribution publication in Pyramid Publish (equivalent to WebFOCUS ReportCaster).
- Manual Review: items requiring human engineer review or verification.
...
Return JSON with exactly these keys:
{
  ...
  "implementation_plan": [{"step_number": 1, "title": "", "action": "",
                            "pyramid_layer": "Data Source / Model Table|Data Flow Step|Semantic Relationship|Calculated Column|Formulate Measure (PQL)|Discover Filter / Slicer|Parameter / Global List|Discover Visual (Grid/Chart)|Present Dashboard|Publish Publication|Manual Review",
...
def generate_ai_build_plan(client, model, payload):
    ...
    while True:
        ...
        raw = normalize_pyramid_layers(raw)
...
def build_pyramid_prep_build_order(lineage_df, plan):
    ...
    sequence matching the order they'd build steps in Pyramid Data Flow or the
...
def render_pyramid_prep_build_order(lineage_df, plan):
    stages = build_pyramid_prep_build_order(lineage_df, plan)
    if not stages:
        st.info("No staging steps detected for this report - it likely reads directly from source with no HOLD chain.")
        return

    st.markdown(
        "Build these as **Pyramid Data Flow flow steps (or Data Source tab steps), in this exact order**. "
...
def render_ai_build_plan(report_row, source_df, lineage_df, join_df, filter_df, formula_df, mapping_df, final_df, field_df):
    st.markdown(
        ...
        "calls - join risk, novel formulas, and worksheet/dashboard suggestions for **Pyramid**."
    )
    ...
    for item in decode_pack:
        with st.container(border=True):
            ...
            if 'lookup_csv' in item:
                ...
                st.caption("Pyramid data source setup:")
...
def suggest_formula_rebuild(formula_type):
    if 'Decode/Mapping' in formula_type:
        return 'Use a Pyramid Formulate PQL Case expression, or a related lookup table in the Data Model.'
    if 'Conditional IF' in formula_type:
        return 'Use a Pyramid Formulate If() expression or PQL Case.'
    if 'Concatenation/String' in formula_type:
        return 'Use Pyramid string functions (SubString, Concat, Replace, Upper, Lower) in a Formulate Calculated Column.'
    if 'Arithmetic' in formula_type:
        return 'Use a Pyramid Formulate custom measure (PQL) or Calculated Column.'
    if 'Parameter-based' in formula_type:
        return 'Use a Pyramid Formulate Global Parameter or Custom List driving a Discover slicer or filter.'
    if 'Date/Time' in formula_type:
        return 'Use Pyramid date functions (DateDiff, DateAdd, Year, Month) in a Formulate column.'
    return 'Review expression and rebuild as a Pyramid Formulate Calculated Column or Custom Measure.'

def suggest_filter_rebuild(expr):
    if extract_parameters(expr):
        return 'Use a Pyramid Formulate Parameter with a Discover slicer or filter driven by it.'
    return 'Use a Pyramid Discover slicer, background filter, or Data Flow row filter.'

def suggest_output_format(fmt):
    fmt = str(fmt or '').upper()
    if fmt in {'HTML', 'HTMTABLE', 'HTMLCSS'}:
        return 'Pyramid Present dashboard published to Pyramid Server, or embedded discovery.'
    if fmt in {'PDF'}:
        return 'Pyramid Publish publication exported/scheduled as PDF.'
    if fmt in {'XLSX', 'EXL2K', 'EXCEL', 'CSV', 'ALPHA', 'DFIX'}:
        return 'Pyramid Discover export as raw data / grid export, or scheduled Publish delivery.'
    if fmt in {'FOCUS', 'HOLD'}:
        return 'Pyramid Data Flow ETL step or intermediate staging table.'
    return 'Map to a Pyramid Discover discovery, Present dashboard, or Publish report based on usage.'


def suggest_join_pyramid_action(from_table, from_key, join_mode, to_table, to_key):
    ...
    action = (
        f"Create a Semantic Model relationship in Pyramid Data Model:\n{from_ref} -> {to_ref}\n\n"
        f"Alternative: Use an ETL Join step in Pyramid Data Flow between {from_table or 'FROM_TABLE'} "
        f"and {to_table or 'TO_TABLE'} on {from_key or 'FROM_KEY'} = {to_key or 'TO_KEY'} if tables "
        f"must be merged physically into a staging dataset before model loading."
    )
    if str(join_mode or '').upper() == 'JOIN TO ALL':
        action += (
            "\n\nWebFOCUS TO ALL suggests multi-row matching may exist. Review key uniqueness, "
            "cardinality, and duplicate row expansion before implementing as a Pyramid relationship."
        )
    return action
...
def suggest_join_review_reason(join_mode):
    if str(join_mode or '').upper() == 'JOIN TO ALL':
        return 'JOIN TO ALL can imply one-to-many or many-to-one behavior. Confirm key uniqueness, cardinality, and row duplication in Pyramid Data Model.'
    return 'Confirm whether this should be a Pyramid Data Model relationship or a Data Flow physical join, and verify key uniqueness/cardinality.'


def suggest_match_pyramid_action(first_table, to_tables, keys, hold):
    ...
    return (
        ...
        f"{hold_text}\n\n"
        "Rebuild as a Pyramid Data Flow ETL step (Merge / Union / Join), or prepare via SQL staging query. "
        "Review OLD/NEW/AFTER/MORE behavior manually before choosing join vs. union in Data Flow."
    )
...
def extract_filter_parameter_mapping(text, defined_names):
    ...
    for match in filter_pattern.finditer(text):
        ...
        rows.append({
            ...
            'pyramid_guidance': suggest_filter_rebuild(expr),
        })
    ...
    for param in extract_parameters(text):
        ...
        parameter_rows.append({
            ...
            'parameters': [param],
            'fields': [],
            'pyramid_guidance': 'Create a Pyramid Formulate Parameter and drive a filter or calculated field from it.',
        })
    ...
    for command, pattern in patterns:
        for match in pattern.finditer(text):
            ...
            rows.append({
                ...
                'format': fmt,
                'pyramid_guidance': suggest_output_format(fmt),
            })
...
def extract_join_match_mapping(text):
    ...
    for match in join_pattern.finditer(text):
        ...
        rows.append({
            ...
            'pyramid_guidance': 'Use a data source relationship or physical join; TO ALL usually maps to one-to-many/many-to-one behavior.',
            'suggested_pyramid_type': 'Data source relationship / physical join',
            'suggested_pyramid_expression_action': suggest_join_pyramid_action(
    ...
    for match in match_block_pattern.finditer(text):
        ...
        rows.append({
            ...
            'pyramid_guidance': 'Use a Pyramid Data Flow union/join or a data source relationship depending on MATCH logic and AFTER/MORE usage.',
            'suggested_pyramid_type': 'Pyramid Data Flow staging flow / union',
            'suggested_pyramid_expression_action': suggest_match_pyramid_action(
    ...
    for pattern in patterns:
        for match in pattern.finditer(text):
            ...
            rows.append({
                ...
                'pyramid_replacement': 'Pyramid Present dashboard action (filter, URL, or highlight action) or a navigation button',
...
def extract_source_inventory(parsed):
    ...
    for source in sources:
        ...
        if source_type in {'CSV', 'Excel', 'Flat file', 'File'}:
            action = f"Connect {layer} as a Pyramid data source from the FILEDEF path."
        elif source_type == 'Database / synonym':
            action = f"Connect {layer} as a Pyramid database/synonym data source connection."
        else:
        ...
        rows.append({
            ...
            'Suggested Pyramid Layer': layer,
            'Suggested Pyramid Action': action,
...
def extract_hold_lineage(text, parsed):
    ...
    for join in parsed.get('join_match_mapping', []):
        ...
        rows.append({
            ...
            'Formulas Created': '',
            'Filters Applied': '',
            'Pyramid Table Suggestion': '',
            'Pyramid Action': join.get('suggested_pyramid_expression_action', ''),
    ...
    for match in extract_table_blocks(clean_text):
        ...
        pyramid_table = suggested_stage_name(table_prefix, output or source)
        action_parts = []
        if output_role in {'Intermediate HOLD', 'Default HOLD'}:
            action_parts.append('Create this as a Pyramid Data Flow flow step / staging data source.')
        else:
        ...
        rows.append({
            ...
            'Pyramid Table Suggestion': pyramid_table,
            'Pyramid Action': ' '.join(action_parts),
    ...
    for match_row in parsed.get('join_match_mapping', []):
        ...
        rows.append({
            ...
            'Formulas Created': '',
            'Filters Applied': '',
            'Pyramid Table Suggestion': suggested_stage_name('stg', ...),
            'Pyramid Action': match_row.get('suggested_pyramid_expression_action', ''),
    ...
    for item in parsed.get('formula_mapping', []):
        ...
        for source_value, mapped_value in pairs:
            rows.append({
                ...
                'Suggested Pyramid Action': f"Create {table_name} as a Pyramid lookup data source and join it on {source_field}.",
            })
...
def recommend_final_dataset(parsed):
    ...
    if has_across or len([m for m in measures.split(', ') if m]) > 1:
        ...
        reason = 'WebFOCUS uses multiple measures or ACROSS-style categories. Pyramid stacked/multi-measure charts usually work best when category/type is stored as rows (long format).'
    else:
    ...
    return [{
        'Recommended Final Dataset': last_step.get('Pyramid Table Suggestion') or 'final_ReportDataset',
        'Dataset Shape': shape,
        'Reason': reason,
        'Suggested Pyramid Visual / Output': 'Pyramid stacked/multi-measure chart' if has_graph else output_type_text(parsed) or 'Pyramid Discover visual/crosstab',
...
def analyze_sql_view_need(parsed):
    ...
    if source_count >= 3 or match_count or join_to_all_count or join_count >= 2 or hold_count >= 3 or score >= 8:
        need = 'Recommended'
        reason = 'Complex JOIN/HOLD/MATCH logic is easier to validate and reuse as a SQL view before connecting Pyramid.'
        path = 'SQL View -> Pyramid Data Model / Prep -> Calculated Fields -> Worksheet'
        action = 'Create a SQL view for joins/HOLD logic, then connect Pyramid and finish shaping/reporting.'
    elif join_count == 1 or hold_count in {1, 2} or complex_formula_count >= 2:
        need = 'Optional'
        reason = 'The report has moderate shaping logic. Pyramid Data Flow / Model can handle it, but a SQL view may be cleaner if reused.'
        path = 'Pyramid Data Model / Prep -> Calculated Fields -> Worksheet'
        action = 'Use Pyramid Data Flow / Model first; consider a SQL view if performance, reuse, or governance is important.'
    else:
        need = 'Not Needed'
        reason = 'The report appears straightforward enough for Pyramid Data Model/Prep and the worksheet layer.'
        path = 'Pyramid Data Model / Prep -> Worksheet'
        action = 'Load source, apply filters/calculations in Pyramid, then build the worksheet/dashboard.'
...
def build_pyramid_plan(parsed):
    ...
    if sql_view['need'] in {'Recommended', 'Optional'}:
        rows.append({
            'Plan Step': step, 'Pyramid Layer': 'SQL View',
    ...
    for source in parsed.get('source_inventory', []):
        rows.append({
            'Plan Step': step, 'Pyramid Layer': source['Suggested Pyramid Layer'],
            'WebFOCUS Object': source['Source Name'],
            'What To Build': source['Suggested Pyramid Action'],
    ...
    for lineage in parsed.get('hold_lineage', []):
        rows.append({
            'Plan Step': step, 'Pyramid Layer': lineage['Pyramid Table Suggestion'],
            'WebFOCUS Object': lineage['Output Table'] or lineage['Input Table(s)'],
            'What To Build': lineage['Pyramid Action'],
    ...
    for mapping in parsed.get('mapping_table_preview', []):
        rows.append({
            'Plan Step': step, 'Pyramid Layer': mapping['Mapping Table Name'],
            'WebFOCUS Object': mapping['Derived Field'],
            'What To Build': mapping['Suggested Pyramid Action'],
    ...
    for final_dataset in parsed.get('final_dataset_plan', []):
        rows.append({
            'Plan Step': step, 'Pyramid Layer': final_dataset['Recommended Final Dataset'],
            'WebFOCUS Object': 'Final output',
            'What To Build': f"Build {final_dataset['Dataset Shape']} dataset for {final_dataset['Suggested Pyramid Visual / Output']}.",
...
def parse_fex(fex_text):
    ...
    for f in result['define_fields']:
        ...
        result['formula_mapping'].append({
            ...
            'pyramid_guidance': suggest_formula_rebuild(formula_type),
        })
...
def build_validation_rows_raw(
    ...
    if sql_view_need == 'Recommended':
        add_check(
            'SQL / Data Validation',
            'Validate SQL view output before connecting Pyramid.',
    ...
    add_check(
        'SQL / Data Validation',
        'Compare final row count between WebFOCUS output and the Pyramid data source.',
        'Always',
    )
    add_check(
...
def build_display_tables(parsed_results, duplicate_records, allowed_program_names, matched_pairs):
    ...
    for folder, fex_name, parsed in parsed_results:
        ...
        sql_view = analyze_sql_view_need(parsed) if parsed is not None else {
            ...
            'action': 'Review parser output before choosing SQL View, Pyramid Data Flow, calculated fields, or the worksheet layer.',
            'score': 0,
        }
        complexity_score = (
        ...
        overview_rows.append({
            ...
            'Pyramid Action': sql_view['action'],
            ...
            'Build Plan Steps': len(parsed['pyramid_build_plan']) if parsed is not None else 0,
        ...
        for item in parsed['filter_parameter_mapping']:
            filter_rows.append({
                ...
                'Pyramid Guidance': item['pyramid_guidance'],
                'Suggested Pyramid Type': item.get('suggested_pyramid_type', ''),
                'Suggested Pyramid Expression / Action': item.get('suggested_pyramid_expression_action', ''),
        ...
        for item in parsed['join_match_mapping']:
            join_rows.append({
                ...
                'Suggested Pyramid Action': item.get('suggested_pyramid_expression_action', ''),
                'Needs Manual Review': item.get('needs_manual_review', ''),
                'Review Reason': item.get('review_reason', ''),
                'Confidence': item.get('confidence', ''),
            })
        ...
        for item in parsed['pyramid_build_plan']:
            row = {'File Path': folder, 'File Name': fex_name}
            row.update(item)
...
st.markdown(
    ...
    <div class="kash-hero">
      ...
      <div class="kash-subtitle">Break Down <span class="kash-subtitle-accent">WebFOCUS</span> Complexity for Faster <span class="kash-subtitle-accent">Pyramid</span> Migration</div>
    </div>
    """,
    unsafe_allow_html=True,
)
...
with st.expander("How to use KashMap", expanded=True):
    st.markdown(
        ...
        4. Review report cards first, then open the guided Pyramid build plan.
...
with st.expander("Translate one WebFOCUS expression or query", expanded=False):
    ...
    if st.button("Translate to Pyramid", key="manual_llm_translate"):
        client = get_openai_client()
        if client is None:
            ...
            st.subheader("Pyramid Translation")
            st.write("**Suggested Pyramid Type**")
            st.write(result.get("suggested_pyramid_type", "") or "Not returned")
            st.write("**Suggested Pyramid Expression / Action**")
            st.code(result.get("suggested_pyramid_expression_action", "") or "Not returned", language="text")
            st.write("**LLM Notes**")
            ...
            st.write(result.get("review_reason", "") or "Review before using in Pyramid.")
            st.write("**Confidence**")
...
def render_overview_story(report_row, source_df, final_df):
    ...
    dataset_name = final_text.get('Recommended Final Dataset', report_row.get('Recommended Final Dataset', 'final Pyramid data source'))
    visual = final_text.get('Suggested Pyramid Visual / Output', report_row.get('Output Format', 'Pyramid Discover visual/dashboard'))
    shape = final_text.get('Dataset Shape', 'Pyramid-ready data model')
    ...
    with st.container(border=True):
        ...
        st.markdown(f"**Pyramid version:** build **{dataset_name}** as a **{shape}**, then use it for the final worksheet or dashboard.")
        st.markdown(
            f"{badge('Build type: Pyramid Present dashboard')} "
...
def source_role_text(source_name, sql_need):
    ...
    if sql_need == 'Optional':
        return 'Pyramid Data Flow / Model or SQL view'
    return 'Load in Pyramid Data Flow / Model'


def recommend_formula_layer(formula_type):
    text = str(formula_type or '')
    if 'Decode/Mapping' in text:
        return 'Lookup data source, then SQL View / Pyramid join'
    if 'Date/Time' in text or 'Concatenation/String' in text or 'Conditional IF' in text:
        return 'SQL View or Pyramid Formulate PQL calculation'
    if 'Arithmetic' in text:
        return 'SQL View / Pyramid Formulate PQL calculation'
    if 'Parameter-based' in text:
        return 'Pyramid Formulate Parameter plus SQL/Data Prep filter'
    return 'Review; likely SQL View or Pyramid Data Flow / Model'
...
def build_start_here_facts(report_row, source_df, lineage_df, join_df, final_df):
    ...
    recommended_layer = 'SQL View' if sql_need == 'Recommended' else 'Pyramid Data Flow / Model'
    if sql_need == 'Optional':
        recommended_layer = 'SQL View or Pyramid Data Flow / Model'
...
def fallback_report_summary(facts):
    ...
    if duplicate_status in {'Exact Duplicate', 'Near Duplicate', 'Same Data Source'}:
        duplicate_text = (f"{duplicate_status} - rebuild once where possible and reuse the same Pyramid approach for this group.")
    else:
        duplicate_text = "Unique - no duplicate group was detected for this report."

    if facts['recommended_layer'] == 'SQL View':
        direction = ("Build the preparation logic in a SQL view first, then connect Pyramid and validate row counts/totals.")
    elif facts['recommended_layer'] == 'SQL View or Pyramid Data Flow / Model':
        direction = ("Use a SQL view or Pyramid Data Flow / Model for the preparation logic, then connect Pyramid and validate row counts/totals.")
    else:
        direction = ("Build the preparation logic in Pyramid Data Flow / Model, then create the Pyramid Discover visual/dashboard and validate row counts/totals.")
...
def render_source_files_tables(source_df, report_row):
    ...
    df['Pyramid Action'] = df['Source Name'].apply(lambda x: source_role_text(x, report_row.get('SQL View Need')))
    show_table_or_info(df[['Source', 'How detected', 'Type', 'Role in Report', 'Pyramid Action']], "No source files/tables were detected.")
...
def render_hold_lineage_inspector(lineage_df, report_row):
    st.write("WebFOCUS HOLD files are temporary datasets. In Pyramid migration, these usually become SQL CTEs, SQL view steps, or Pyramid Data Flow staging flow steps.")
    ...
    hold_df['Migration equivalent'] = hold_df['Output Role'].apply(
        lambda role: 'SQL CTE inside SQL view' if report_row.get('SQL View Need') == 'Recommended' else 'Pyramid Data Flow staging step'
    )
...
def render_joins_matches_inspector(join_df, report_row):
    ...
    if report_row.get('SQL View Need') == 'Recommended':
        df['Recommended Pyramid/SQL Action'] = 'Implement inside SQL view first. Use a Pyramid data source relationship only if the joined table remains separate.'
    else:
        df['Recommended Pyramid/SQL Action'] = df['Suggested Pyramid Action']
    df['Review'] = df['Review Reason']
    show_table_or_info(df[['Risk', 'Join / Match', 'Left Side', 'Right Side', 'Recommended Pyramid/SQL Action', 'Review']], "No JOIN/MATCH statements were detected.")
...
def render_filters_parameters_inspector(filter_df):
    ...
    df['Pyramid Treatment'] = df.apply(short_filter_treatment, axis=1)
    field_col = 'Fields Used' if 'Fields Used' in df.columns else 'Fields'
    show_table_or_info(df[['Filter / Parameter', field_col, 'Meaning', 'Pyramid Treatment']], "No filters or parameters were detected.")
...
def short_filter_treatment(row):
    ...
    if filter_type == 'PARAMETER' or parameter.strip():
        return 'Pyramid Formulate Parameter'
    if filter_type == 'WHERE':
        return 'Fixed SQL filter' if 'SQL' in str(row.get('Pyramid Guidance', '')).upper() else 'Pyramid Data Flow row filter'
    if filter_type == 'IF':
        return 'SQL WHERE / Pyramid Data Flow row filter'
    if '&' in expression:
        return 'Pyramid Formulate Parameter'
...
def sql_view_reasons(report_row, source_df, lineage_df, join_df):
    ...
    if report_row.get('SQL View Need') == 'Not Needed':
        reasons.append("Simple enough for Pyramid Data Flow / Model based on current parser rules")
    return reasons
...
def render_sql_view_recommendation_inspector(report_row, source_df, lineage_df, join_df, filter_df=None, formula_df=None):
    ...
    st.write("**Preferred:** create this as a database view, then connect Pyramid's data source directly to the view.")
    st.write("**Alternative:** use Pyramid's custom SQL connection with the final reviewed SELECT query if database permissions do not allow creating views.")
...
def render_pyramid_approach_inspector(report_row, build_plan_df):
    need = report_row.get('SQL View Need', 'Needs Review')
    view_name = f"vw_{Path(str(report_row.get('FEX Name', 'report'))).stem.lower()}_base"
    if need == 'Recommended':
        st.write("**Recommended approach:** SQL View -> Pyramid Data Model/Prep -> Dashboard")
        steps = [
            f"Create SQL View: ask the DBA/migration developer to create `{view_name}`.",
            f"Connect Pyramid Analytics to `{view_name}`.",
        ...
        st.write("**Recommended approach:** Pyramid Data Model/Prep -> Dashboard")
        steps = [
            "Connect Pyramid to the source file/table.",
            "Use Pyramid Data Flow / Model for filters, type cleanup, and report-specific transformations.",
...
def render_drilldown_output_inspector(report_row, final_df, drilldown_df, lineage_df=None):
    ...
    st.markdown(f"**Pyramid Output Recommendation:** {output_recommendation}")
    hold_outputs = []
    ...
    if drilldown_df is None or drilldown_df.empty:
        ...
        df['Pyramid replacement'] = df['pyramid_replacement']
        show_table_or_info(df[['WebFOCUS drilldown', 'Target report', 'Parameter', 'Pyramid replacement']], "No drilldown detected.")


def render_data_prep_inspector(report_row):
    sql_needed = report_row.get('SQL View Need') == 'Recommended'
    if sql_needed:
        st.write("Pyramid Data Flow / Model should be light cleanup only.")
        st.info(f"Because a SQL View is recommended, Pyramid Data Flow should not rebuild the full WebFOCUS logic. Use it only after `{sql_view_name(report_row)}` is created and validated.")
        steps = [
            f"Connect Pyramid data source to {sql_view_name(report_row)}", "Confirm preview loads", "Set data types",
            "Rename business-facing fields", "Remove technical/helper columns", "Keep final report fields",
            "Publish or embed the data source",
        ]
    else:
        st.write("Pyramid Data Flow should recreate the WebFOCUS staging flow.")
        steps = [
...
def render_calculated_field_inspector(report_row, field_df, formula_df):
    st.info("Do not move all WebFOCUS DEFINE formulas into calculated fields. Most row-level formulas should be handled in SQL View or Pyramid Data Flow.")
    candidates = []
    ...
    for field in list(dict.fromkeys(candidates))[:8]:
        ...
        st.code(f"// Pyramid Formulate PQL calculation: {measure_name}\nSUM([{field}])", language='text')
...
def render_visuals_inspector(report_row, final_df, field_df):
    ...
    st.markdown(f"**Recommended Pyramid Present Dashboard:** {Path(str(report_row.get('FEX Name', 'report'))).stem.replace('_', ' ').title()}")
...
def render_detailed_report_summary(report_row, source_df, lineage_df, join_df, filter_df, formula_df):
    ...
    if sql_need == 'Recommended':
        summary += (
            ...
            "a SQL view that reproduces the WebFOCUS data preparation logic. Pyramid should connect to the SQL view "
            "and use Pyramid Data Flow only for light cleanup such as data types, naming, and final field selection. "
        )
    else:
        summary += (
            "Based on the detected logic, this can likely be migrated with Pyramid Data Flow / Model shaping and a Pyramid Discover visual. "
        )
...
def render_walkthrough(lineage_df):
    ...
    for _, row in lineage_df.sort_values('Step #').iterrows():
        ...
        with st.expander(step_title, expanded=int(row['Step #']) <= 3):
            ...
            st.write(f"Pyramid action: {row['Pyramid Action'] or 'Review this step and rebuild it in Pyramid Data Flow or the data source.'}")
            if row.get('Output Table'):
                st.write(f"Output table: {row['Output Table']} ({row['Output Role']})")
            if row.get('Pyramid Table Suggestion'):
                st.write(f"Suggested Pyramid table/data source: {row['Pyramid Table Suggestion']}")
...
def render_pipeline(lineage_df, final_df):
    ...
    for _, row in lineage_df.sort_values('Step #').iterrows():
        ...
        output_value = str(row.get('Output Table', '') or row.get('Pyramid Table Suggestion', '') or '').strip()
    ...
    if not final_df.empty:
        ...
        lines.append(f"   -> {final_row.get('Suggested Pyramid Visual / Output', 'Pyramid Discover visual')}")

    st.code('\n'.join(lines).strip(), language='text')


def render_pyramid_prep_checklist(build_plan_df):
    if build_plan_df.empty:
        st.info("No Pyramid Data Flow checklist was generated.")
        return
    prep_rows = build_plan_df[
        build_plan_df['What To Build'].astype(str).str.contains('Pyramid Data Flow|staging|join|source query|calculated field|unpivot', case=False, na=False)
    ]
    if prep_rows.empty:
        prep_rows = build_plan_df
    for _, row in prep_rows.sort_values('Plan Step').iterrows():
        label = f"Step {row['Plan Step']}: {row['Pyramid Layer'] or row['WebFOCUS Object']}"
...
def render_mapping_cards(mapping_df):
    ...
    for table_name, group in mapping_df.groupby('Mapping Table Name'):
        with st.expander(f"{table_name} ({len(group)} rows detected)", expanded=False):
            ...
            st.write(first['Suggested Pyramid Action'])
            cols = [
                'Mapping Table Name',
                'Source Field',
                'Source Value',
...
def render_visual_plan(final_df):
    ...
    for _, row in final_df.iterrows():
        with st.container(border=True):
            ...
            st.markdown(f"**Visual/output:** {row['Suggested Pyramid Visual / Output']}")
...
def collect_review_items(build_plan_df, joins_df, filters_df, formulas_df):
    ...
    if not build_plan_df.empty:
        for _, row in build_plan_df[build_plan_df['Manual Review'].astype(str).eq('Yes')].iterrows():
            items.append(('Medium', row['Pyramid Layer'] or row['WebFOCUS Object'], row['Why'], row['What To Build']))
    ...
    if not joins_df.empty:
        for _, row in joins_df.iterrows():
            ...
            items.append((priority, 'Join key check', row.get('Review Reason', 'Confirm join behavior.'), row.get('Suggested Pyramid Action', 'Review join.')))
    ...
    if not filters_df.empty:
        ...
        for _, row in risky_filters.iterrows():
            items.append(('Medium', 'Parameter/filter check', row['Expression'], row['Pyramid Guidance']))
    ...
    if not formulas_df.empty:
        ...
        for _, row in risky_formulas.iterrows():
            ...
            items.append((priority, f"Formula: {row['Field Name']}", row['Raw WebFOCUS Formula'], row['Suggested Pyramid Expression / Action'] or 'Review formula rebuild.'))
...
def render_sql_view_plan(build_plan_df, report_row):
    ...
    if build_plan_df is not None and not build_plan_df.empty:
       sql_rows = build_plan_df[build_plan_df['Pyramid Layer'].astype(str).str.contains('SQL View', case=False, na=False)]

    if sql_rows.empty and report_row.get('SQL View Need') == 'Not Needed':
        st.write("This report appears suitable for Pyramid Data Flow / Model and the worksheet layer.")
        return
    ...
    st.markdown(
        ...
        <div class="wizard-card">
          ...
          <p><strong>Action:</strong> {report_row.get('Pyramid Action', 'Review SQL View need.')}</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    if sql_rows.empty:
        ...
        for _, row in sql_rows.iterrows():
            with st.expander(f"Step {row['Plan Step']}: {row['Pyramid Layer']}", expanded=True):
...
def render_calc_field_plan(formula_df, final_df):
    if formula_df is None or formula_df.empty:
        st.info("No calculated-field-specific formulas were detected. This may be mostly Pyramid Data Flow and worksheet setup.")
        return
    ...
    if calc_candidates.empty:
        st.info("No clear calculated field candidates were detected. Use Pyramid Data Flow for source shaping and mapping logic.")
        return
    st.write("Review these formulas and decide whether they belong in Pyramid Data Flow custom steps or worksheet calculated fields.")
    show_table_or_info(
        calc_candidates[['Field Name', 'Formula Type', 'Raw WebFOCUS Formula', 'Suggested Pyramid Expression / Action', 'Needs Manual Review']],
...
def render_validation_checklist(report_row, join_df, final_df, drilldown_df=None):
    ...
    st.caption("Use this before marking the report as rebuilt in Pyramid. Work through each group, tick completed checks, and add tester notes if needed.")
    ...
    st.text_area(
        ...
        placeholder="Example: WebFOCUS row count = 12,450; Pyramid row count = 12,450. Totals matched.",
        height=110,
    )
...
def render_sources_hold_combined(source_df, lineage_df, report_row):
    ...
    st.markdown('<div class="tab-intro">Confirm which files and lookup tables feed the report before rebuilding it in Pyramid.</div>', unsafe_allow_html=True)
...
def render_joins_filters_combined(join_df, filter_df, report_row):
    ...
    with st.expander("Show raw filter expressions"):
        cols = [col for col in ['Filter Type', 'Expression', 'Parameter', 'Fields Used', 'Pyramid Guidance'] if col in filter_df.columns]
        show_table_or_info(
...
def render_joins_tab(join_df, report_row):
    st.subheader("Joins")
    st.markdown('<div class="tab-intro">Review JOIN and MATCH logic, keys, and whether the relationship belongs in SQL, Pyramid Data Flow, or the Pyramid data source.</div>', unsafe_allow_html=True)
    render_joins_matches_inspector(join_df, report_row)
    with st.expander("Show raw JOIN/MATCH statements"):
        cols = [col for col in ['Mapping Type', 'From Table', 'From Key', 'To Table', 'To Key', 'Raw WebFOCUS Statement', 'Review Reason'] if col in join_df.columns]
...
def render_filters_tab(filter_df):
    ...
    st.markdown('<div class="tab-intro">Translate WebFOCUS WHERE, IF, and parameter logic into Pyramid Data Flow filters, dashboard filters, or calculated field rules.</div>', unsafe_allow_html=True)
    render_filters_parameters_inspector(filter_df)
    with st.expander("Show raw filter expressions"):
        cols = [col for col in ['Filter Type', 'Expression', 'Parameter', 'Fields Used', 'Pyramid Guidance'] if col in filter_df.columns]
