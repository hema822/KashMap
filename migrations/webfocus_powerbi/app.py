...
def generate_decode_dax_switch(source_field, derived_field, pairs, final_table):
    lines = [f"{derived_field} =", "SWITCH (", "    TRUE(),"]
    for source_value, mapped_value in pairs:
        safe_source = source_value.replace('"', '""')
        safe_mapped = mapped_value.replace('"', '""')
        lines.append(f'    {final_table}[{source_field}] = "{safe_source}", "{safe_mapped}",')
...
def generate_decode_power_query_merge_step(source_field, derived_field, lookup_table_name):
    return (
        f"let\n"
        f"    Source = <PreviousStepName>,\n"
        f"    MergedWithLookup = Table.NestedJoin(\n"
        f"        Source, {{\"{source_field}\"}},\n"
        f"        {lookup_table_name}, {{\"{source_field}\"}},\n"
        f"        \"{lookup_table_name}\", JoinKind.LeftOuter\n"
        f"    ),\n"
        f"    #\"Expanded {derived_field}\" = Table.ExpandTableColumn(\n"
        f"        MergedWithLookup, \"{lookup_table_name}\",\n"
        f"        {{\"{derived_field}\"}}, {{\"{derived_field}\"}}\n"
        f"    )\n"
        f"in\n"
        f"    #\"Expanded {derived_field}\""
    )
    ...
    for table_name, group in mapping_df.groupby('Mapping Table Name'):
        ...
        if len(pairs) <= switch_threshold:
            packs.append({
                ...
                'recommended_approach': 'DAX SWITCH (inline)',
                'code': generate_decode_dax_switch(source_field, derived_field, pairs, final_table_name),
            })
        else:
            ...
            packs.append({
                ...
                'derived_field': derived_field,
                'pair_count': len(pairs),
                'recommended_approach': f'Power Query lookup table ({lookup_name})',
                'lookup_csv': generate_decode_lookup_table_csv(source_field, derived_field, pairs),
                'merge_code': generate_decode_power_query_merge_step(source_field, derived_field, lookup_name),
            })
            ...
            if count >= min_branches:
                results.append({
                    ...
                    'suggestion': (
                        ...
                        'or directly to a Power BI lookup table.'
                    ),
                })
    return results
...
def build_rule_findings(parsed, mapping_df, final_table_name):
    ...
    if date_idiom:
        findings.append({
            ...
            'recommendation': 'Convert positional date-string parsing into a typed Date column in Power Query.',
...
def translate_expression_with_llm(client, model, expression_type, expression, context):
    ...
    prompt = f"""
You translate WebFOCUS migration logic into Power BI guidance.
...
Rules:
- The value for suggested_power_bi_expression_action must be one plain multiline string.
- Do not return suggested_power_bi_expression_action as a JSON array, Python list, bullet list, or numbered list.
- If there are multiple calculated columns, format them in one multiline string like:
  PROFIT =
  SALES[SALES_AMOUNT] - SALES[COST_AMOUNT]

  SALES_LABEL =
  SALES[REGION] & " - " & SALES[PRODUCT_CATEGORY]
- Prefer DAX calculated column for row-level DEFINE/COMPUTE logic.
- Prefer DAX measure only when aggregation context is clearly needed.
- Prefer Power Query M for source shaping, filters before load, joins, merges, and cleanup.
- For JOIN or MATCH, explain the likely Power Query merge or Power BI relationship approach.
...
def _numeric_measure_candidates(field_df):
    """Reuses the same value-field heuristic as the rule-based DAX inspector,
    so the AI payload and the rule-based fallback stay consistent."""
    candidates = []
...
AI_BUILD_PLAN_SYSTEM_PROMPT = """You are a senior BI migration engineer converting one parsed WebFOCUS report to Power BI.
...
# Priority order when the model returns a combined value like "Source SQL|Power Query" -
# pick the first one that appears in the combined string, in this precedence order.
_LAYER_PRECEDENCE = [
    "Power Query", "Source SQL", "Mapping Table", "Semantic Model",
    "DAX Measure", "Calculated Column", "Report Filter", "Slicer",
    "Visual", "Drill-through", "Paginated Report", "Manual Review",
]
...
def _coerce_power_bi_layer(value):
    if value in VALID_POWER_BI_LAYERS:
        return value
    # Model combined two or more valid values, e.g. "Source SQL|Power Query"
    # or "DAX Measure|Calculated Column" - pick one deterministically.
    ...
    if parts:
        for preferred in _LAYER_PRECEDENCE:
            if preferred in parts:
                return preferred
        return parts[0]
    return "Manual Review"  # unrecognized value entirely - safest fallback


def normalize_power_bi_layers(raw):
    """Fixes cases where the model returns a combined power_bi_layer value
    (e.g. 'Source SQL|Power Query') instead of picking one valid Literal,
...
def build_power_query_build_order(lineage_df, plan):
    """Combines the HOLD chain's real execution order (from lineage_df) with
    the AI plan's Power Query steps, so the report maker gets one numbered
    sequence matching exactly the order they'd click 'Add Step' in Power
    Query - instead of hunting through separate Data Prep/Calculation cards."""
    ordered_stages = []
    ...
    if plan is not None:
        pq_steps = [
            ...
            if step.power_bi_layer == "Power Query"
        ]
        for step in pq_steps:
            matched = False
...
def render_power_query_build_order(lineage_df, plan):
    stages = build_power_query_build_order(lineage_df, plan)
    if not stages:
        st.info("No staging steps detected for this report - it likely reads directly from source with no HOLD chain.")
        return

    st.markdown(
        "Build these as Power Query **Applied Steps in this exact order**. "
...
def render_ai_build_plan(report_row, source_df, lineage_df, join_df, filter_df, formula_df, mapping_df, final_df, field_df):
    st.markdown(
        ...
        "calls - JOIN risk, novel formulas, and page/visual suggestions."
    )
    ...
    with tabs[0]:
        render_power_query_build_order(lineage_df, plan)
...
def suggest_formula_rebuild(formula_type):
    if 'Decode/Mapping' in formula_type:
        return 'Use Power Query conditional column, lookup table merge, or DAX SWITCH.'
    if 'Conditional IF' in formula_type:
        return 'Use DAX IF/SWITCH or Power Query conditional column.'
    ...
    if 'Arithmetic' in formula_type:
        return 'Use a DAX calculated column/measure or Power Query custom column.'
    if 'Parameter-based' in formula_type:
        return 'Use Power BI parameters, slicers, or dynamic M query parameters.'
    if 'Date/Time' in formula_type:
        return 'Use Power Query date functions or DAX date/time functions.'
    return 'Review expression and rebuild as DAX or Power Query M.'


def suggest_filter_rebuild(expr):
    if extract_parameters(expr):
        return 'Use Power BI parameters, slicers, or filter pane logic.'
    return 'Use report/page filters, visual filters, or Power Query row filters.'


def suggest_output_format(fmt):
    fmt = str(fmt or '').upper()
    if fmt in {'HTML', 'HTMTABLE', 'HTMLCSS'}:
        return 'Power BI report page or published app.'
    if fmt in {'PDF'}:
        return 'Power BI export to PDF or paginated report.'
    if fmt in {'XLSX', 'EXL2K', 'EXCEL', 'CSV', 'ALPHA', 'DFIX'}:
        return 'Power BI export data, Power Query output, or paginated report export.'
    if fmt in {'FOCUS', 'HOLD'}:
        return 'Power Query staging table/dataflow or semantic model table.'
    return 'Map to Power BI report, export, or dataflow based on usage.'


def suggest_join_power_bi_action(from_table, from_key, join_mode, to_table, to_key):
    ...
    action = (
        ...
        f"Alternative: Power Query Merge {from_table or 'FROM_TABLE'} with {to_table or 'TO_TABLE'} "
        f"on {from_key or 'FROM_KEY'} = {to_key or 'TO_KEY'} if columns from {to_table or 'TO_TABLE'} "
        f"must be physically added before reporting."
    )

    if str(join_mode or '').upper() == 'JOIN TO ALL':
        action += (
            "\n\nWebFOCUS TO ALL suggests multi-row matching may exist. Review key uniqueness, "
            "cardinality, and filter direction before implementing."
        )
...
def suggest_join_review_reason(join_mode):
    if str(join_mode or '').upper() == 'JOIN TO ALL':
        return 'JOIN TO ALL can imply one-to-many or many-to-one behavior. Confirm key uniqueness, cardinality, and filter direction.'
    return 'Confirm whether this should be a model relationship or a Power Query merge, and verify key uniqueness/cardinality.'


def suggest_match_power_bi_action(first_table, to_tables, keys, hold):
    ...
    return (
        ...
        f"{hold_text}\n\n"
        "Rebuild as a Power Query staging query or dataflow. Review OLD/NEW/AFTER/MORE behavior "
        "manually before choosing merge vs append."
    )
...
def extract_filter_parameter_mapping(text, defined_names):
    ...
    for match in filter_pattern.finditer(text):
        ...
        rows.append({
            ...
            'power_bi': suggest_filter_rebuild(expr),
        })
    ...
    for param in extract_parameters(text):
        ...
        parameter_rows.append({
            ...
            'parameters': [param],
            'fields': [],
            'power_bi': 'Create a Power BI parameter, slicer, or dynamic M query parameter.',
        })
    ...
    for command, pattern in patterns:
        for match in pattern.finditer(text):
            ...
            rows.append({
                ...
                'format': fmt,
                'power_bi': suggest_output_format(fmt),
            })
...
def extract_join_match_mapping(text):
    ...
    for match in join_pattern.finditer(text):
        ...
        rows.append({
            ...
            'power_bi': 'Use model relationship or Power Query merge; TO ALL usually maps to one-to-many/many-to-one relationship behavior.',
            'suggested_power_bi_type': 'Model relationship / Power Query merge',
            'suggested_power_bi_expression_action': suggest_join_power_bi_action(
                from_table,
                from_key,
                join_type,
                to_table,
    ...
    for match in match_block_pattern.finditer(text):
        ...
        rows.append({
            ...
            'power_bi': 'Use Power Query append/merge or model relationships depending on MATCH logic and AFTER/MORE usage.',
            'suggested_power_bi_type': 'Power Query staging query / dataflow',
            'suggested_power_bi_expression_action': suggest_match_power_bi_action(
                first_table,
    ...
    for pattern in patterns:
        for match in pattern.finditer(text):
            ...
            rows.append({
                ...
                'power_bi_replacement': 'Power BI drillthrough page or page navigation button',
...
def extract_source_inventory(parsed):
    ...
    for source in sources:
        ...
        if source_type in {'CSV', 'Excel', 'Flat file', 'File'}:
            action = f"Create Power Query source query {layer} from the FILEDEF path."
        elif source_type == 'Database / synonym':
            action = f"Create Power Query or SQL source query {layer} from the database/synonym reference."
        else:
            action = f"Create Power Query staging query {layer}; confirm the synonym's physical source."
        ...
        rows.append({
            ...
            'Suggested Power BI Layer': layer,
            'Suggested Power BI Action': action,
...
def extract_hold_lineage(text, parsed):
    ...
    for join in parsed.get('join_match_mapping', []):
        ...
        rows.append({
            ...
            'Formulas Created': '',
            'Filters Applied': '',
            'Power BI Table Suggestion': '',
            'Power BI Action': join.get('suggested_power_bi_expression_action', ''),
    ...
    for match in extract_table_blocks(clean_text):
        ...
        power_bi_table = suggested_stage_name(table_prefix, output or source)
        action_parts = []

        if output_role in {'Intermediate HOLD', 'Default HOLD'}:
            action_parts.append('Create this as a Power Query staging query/dataflow step.')
        else:
            action_parts.append('Use this as the final semantic model table or visual dataset.')
        if measures:
        ...
        rows.append({
            ...
            'Power BI Table Suggestion': power_bi_table,
            'Power BI Action': ' '.join(action_parts),
    ...
    for match_row in parsed.get('join_match_mapping', []):
        ...
        rows.append({
            ...
            'Formulas Created': '',
            'Filters Applied': '',
            'Power BI Table Suggestion': suggested_stage_name('stg', match_row.get('output_hold') or match_row.get('from_table')),
            'Power BI Action': match_row.get('suggested_power_bi_expression_action', ''),
    ...
    for item in parsed.get('formula_mapping', []):
        ...
        for source_value, mapped_value in pairs:
            rows.append({
                ...
                'Suggested Power BI Action': f"Create {table_name} as a mapping table and merge it in Power Query on {source_field}.",
            })
...
def recommend_final_dataset(parsed):
    ...
    if has_across or len([m for m in measures.split(', ') if m]) > 1:
        ...
        reason = 'WebFOCUS uses multiple measures or ACROSS-style categories. Power BI stacked charts usually work best when category/type is stored as rows.'
    else:
    ...
    return [{
        'Recommended Final Dataset': last_step.get('Power BI Table Suggestion') or 'final_ReportDataset',
        'Dataset Shape': shape,
        'Reason': reason,
        'Suggested Power BI Visual / Output': 'Power BI chart visual' if has_graph else output_type_text(parsed) or 'Power BI report visual/table',
...
def analyze_sql_view_need(parsed):
    ...
    if source_count >= 3 or match_count or join_to_all_count or join_count >= 2 or hold_count >= 3 or score >= 8:
        need = 'Recommended'
        reason = 'Complex JOIN/HOLD/MATCH logic is easier to validate and reuse as a SQL view before Power BI.'
        path = 'SQL View -> Power Query -> DAX -> Visual'
        action = 'Create SQL view for joins/HOLD logic, then connect Power BI and finish shaping/reporting.'
    elif join_count == 1 or hold_count in {1, 2} or complex_formula_count >= 2:
        need = 'Optional'
        reason = 'The report has moderate shaping logic. Power Query can handle it, but SQL view may be cleaner if reused.'
        path = 'Power Query -> DAX -> Visual'
        action = 'Use Power Query first; consider SQL view if performance, reuse, or governance is important.'
    else:
        need = 'Not Needed'
        reason = 'The report appears straightforward enough for Power Query and the Power BI visual layer.'
        path = 'Power Query -> Visual'
        action = 'Load source, apply filters/calculations in Power Query, then build the report visual.'

    if 'GRAPH' in output_formats and 'Visual' not in path:
        path = f"{path} -> Visual"
...
def build_power_bi_plan(parsed):
    rows = []
    step = 1
    ...
    for source in parsed.get('source_inventory', []):
        rows.append({
            'Plan Step': step,
            'Power BI Layer': source['Suggested Power BI Layer'],
            'WebFOCUS Object': source['Source Name'],
            'What To Build': source['Suggested Power BI Action'],
            'Why': 'Start with source/staging queries before rebuilding HOLD outputs.',
    ...
    for lineage in parsed.get('hold_lineage', []):
        rows.append({
            'Plan Step': step,
            'Power BI Layer': lineage['Power BI Table Suggestion'],
            'WebFOCUS Object': lineage['Output Table'] or lineage['Input Table(s)'],
            'What To Build': lineage['Power BI Action'],
    ...
    for mapping in parsed.get('mapping_table_preview', []):
        rows.append({
            'Plan Step': step,
            'Power BI Layer': mapping['Mapping Table Name'],
            'WebFOCUS Object': mapping['Derived Field'],
            'What To Build': mapping['Suggested Power BI Action'],
            'Why': 'DECODE logic is easier to maintain as a lookup/mapping table.',
            'Manual Review': 'Yes',
        })
        step += 1
    ...
    for final_dataset in parsed.get('final_dataset_plan', []):
        rows.append({
            'Plan Step': step,
            'Power BI Layer': final_dataset['Recommended Final Dataset'],
            'WebFOCUS Object': 'Final output',
            'What To Build': f"Build {final_dataset['Dataset Shape']} dataset for {final_dataset['Suggested Power BI Visual / Output']}.",
...
def parse_fex(fex_text):
    ...
    for f in result['define_fields']:
        ...
        result['formula_mapping'].append({
            ...
            'power_bi': suggest_formula_rebuild(formula_type),
        })
    ...
    result['power_bi_build_plan'] = build_power_bi_plan(result)
...
def build_validation_rows_raw(
    ...
    if sql_view_need == 'Recommended':
        add_check(
            'SQL / Data Validation',
            'Validate SQL view output before connecting Power BI.',
    ...
    add_check(
        'SQL / Data Validation',
        'Compare final row count between WebFOCUS output and Power BI dataset.',
        'Always',
    )
    add_check(
...
def build_display_tables(parsed_results, duplicate_records, allowed_program_names, matched_pairs):
    ...
    for folder, fex_name, parsed in parsed_results:
        ...
        sql_view = analyze_sql_view_need(parsed) if parsed is not None else {
            ...
            'action': 'Review parser output before choosing SQL View, Power Query, DAX, or visual layer.',
            'score': 0,
        }
        complexity_score = (
        ...
        overview_rows.append({
            ...
            'Power BI Action': sql_view['action'],
            ...
            'Build Plan Steps': len(parsed['power_bi_build_plan']) if parsed is not None else 0,
        ...
        for item in parsed['filter_parameter_mapping']:
            filter_rows.append({
                ...
                'Power BI Guidance': item['power_bi'],
                'Suggested Power BI Type': item.get('suggested_power_bi_type', ''),
                'Suggested Power BI Expression / Action': item.get('suggested_power_bi_expression_action', ''),
        ...
        for item in parsed['join_match_mapping']:
            join_rows.append({
                ...
                'Suggested Power BI Action': item.get('suggested_power_bi_expression_action', ''),
                'Needs Manual Review': item.get('needs_manual_review', ''),
                'Review Reason': item.get('review_reason', ''),
                'Confidence': item.get('confidence', ''),
            })
        ...
        for item in parsed['power_bi_build_plan']:
            row = {'File Path': folder, 'File Name': fex_name}
            row.update(item)
...
st.markdown(
    ...
    <div class="kash-hero">
      ...
      <div class="kash-subtitle">Break Down <span class="kash-subtitle-accent">WebFOCUS</span> Complexity for Faster <span class="kash-subtitle-accent">Power BI</span> Migration</div>
    </div>
    """,
    unsafe_allow_html=True,
)
...
with st.expander("How to use KashMap", expanded=True):
    st.markdown(
        ...
        4. Review report cards first, then open the guided Power BI build plan.
...
def render_overview_story(report_row, source_df, final_df):
    ...
    dataset_name = final_text.get('Recommended Final Dataset', report_row.get('Recommended Final Dataset', 'final Power BI dataset'))
    visual = final_text.get('Suggested Power BI Visual / Output', report_row.get('Output Format', 'Power BI visual/table'))
    shape = final_text.get('Dataset Shape', 'Power BI-ready dataset')
    ...
    with st.container(border=True):
        ...
        st.markdown(
            f"**Power BI version:** build **{dataset_name}** as a **{shape}**, "
            f"then use it for the final report page or visual."
        )
        st.markdown(
            f"{badge('Build type: Power BI report')} "
...
def source_role_text(source_name, sql_need):
    ...
    if sql_need == 'Optional':
        return 'Power Query or SQL view'
    return 'Load in Power Query'


def recommend_formula_layer(formula_type):
    text = str(formula_type or '')
    if 'Decode/Mapping' in text:
        return 'Mapping table, then SQL View / Power Query merge'
    if 'Date/Time' in text or 'Concatenation/String' in text or 'Conditional IF' in text:
        return 'SQL View or Power Query calculated column'
    if 'Arithmetic' in text:
        return 'SQL View / Power Query calculated column'
    if 'Parameter-based' in text:
        return 'Power BI slicer/parameter plus SQL/Power Query filter'
    return 'Review; likely SQL View or Power Query'
...
def build_start_here_facts(report_row, source_df, lineage_df, join_df, final_df):
    ...
    recommended_layer = 'SQL View' if sql_need == 'Recommended' else 'Power Query'
    if sql_need == 'Optional':
        recommended_layer = 'SQL View or Power Query'
...
def fallback_report_summary(facts):
    ...
    if duplicate_status in {'Exact Duplicate', 'Near Duplicate', 'Same Data Source'}:
        duplicate_text = (
            ...
            "Power BI approach for this group."
        )
    else:
    ...
    if facts['recommended_layer'] == 'SQL View':
        direction = (
            "Build the preparation logic in a SQL view first, then connect Power BI "
            "and validate row counts/totals."
        )
    elif facts['recommended_layer'] == 'SQL View or Power Query':
        direction = (
            "Use a SQL view or Power Query for the preparation logic, then connect Power BI "
            "and validate row counts/totals."
        )
    else:
        direction = (
            "Build the preparation logic in Power Query, then create the Power BI report "
            "and validate row counts/totals."
        )
...
def render_source_files_tables(source_df, report_row):
    ...
    df['Power BI Action'] = df['Source Name'].apply(lambda x: source_role_text(x, report_row.get('SQL View Need')))
    show_table_or_info(df[['Source', 'How detected', 'Type', 'Role in Report', 'Power BI Action']], "No source files/tables were detected.")
...
def render_hold_lineage_inspector(lineage_df, report_row):
    st.write("WebFOCUS HOLD files are temporary datasets. In Power BI migration, these usually become SQL CTEs, SQL view steps, or Power Query staging queries.")
    ...
    hold_df['Migration equivalent'] = hold_df['Output Role'].apply(
        lambda role: 'SQL CTE inside SQL view' if report_row.get('SQL View Need') == 'Recommended' else 'Power Query staging query'
    )
...
def render_joins_matches_inspector(join_df, report_row):
    ...
    if report_row.get('SQL View Need') == 'Recommended':
        df['Recommended Power BI/SQL Action'] = 'Implement inside SQL view first. Use Power BI relationship only if the joined table remains separate.'
    else:
        df['Recommended Power BI/SQL Action'] = df['Suggested Power BI Action']
    df['Review'] = df['Review Reason']
    show_table_or_info(df[['Risk', 'Join / Match', 'Left Side', 'Right Side', 'Recommended Power BI/SQL Action', 'Review']], "No JOIN/MATCH statements were detected.")
...
def render_filters_parameters_inspector(filter_df):
    ...
    df['Power BI Treatment'] = df.apply(short_filter_treatment, axis=1)
    field_col = 'Fields Used' if 'Fields Used' in df.columns else 'Fields'
    show_table_or_info(df[['Filter / Parameter', field_col, 'Meaning', 'Power BI Treatment']], "No filters or parameters were detected.")
...
def short_filter_treatment(row):
    ...
    if filter_type == 'PARAMETER' or parameter.strip():
        return 'Power BI slicer / parameter'
    if filter_type == 'WHERE':
        return 'Fixed SQL filter' if 'SQL' in str(row.get('Power BI Guidance', '')).upper() else 'Power Query row filter'
    if filter_type == 'IF':
        return 'SQL WHERE / Power Query row filter'
    if '&' in expression:
        return 'Power BI slicer / parameter'
...
def sql_view_reasons(report_row, source_df, lineage_df, join_df):
    ...
    if report_row.get('SQL View Need') == 'Not Needed':
        reasons.append("Simple enough for Power Query based on current parser rules")
    return reasons
...
def render_sql_view_recommendation_inspector(report_row, source_df, lineage_df, join_df, filter_df=None, formula_df=None):
    ...
    st.write("**Preferred:** create this as a database view, then connect Power BI to the view.")
    st.write("**Alternative:** paste the final reviewed SELECT query into Power BI native SQL query if database permissions do not allow creating views.")
...
def render_power_bi_approach_inspector(report_row, build_plan_df):
    need = report_row.get('SQL View Need', 'Needs Review')
    view_name = f"vw_{Path(str(report_row.get('FEX Name', 'report'))).stem.lower()}_base"
    if need == 'Recommended':
        st.write("**Recommended approach:** SQL View -> Power BI Power Query -> Report Page")
        steps = [
            f"Create SQL View: ask the database/report migration developer to create `{view_name}`.",
            f"Connect Power BI Desktop to `{view_name}`.",
            "Clean in Power Query: set data types, rename columns, remove unused columns, and keep report-needed fields.",
            "Build report page: create the final table, matrix, or chart visual.",
            "Validate: compare row counts, totals, filters, and join results against WebFOCUS output.",
        ]
    else:
        st.write("**Recommended approach:** Power Query -> Report Page")
        steps = [
            "Connect Power BI to the source file/table.",
            "Use Power Query for filters, type cleanup, and report-specific transformations.",
            "Create DAX measures only if final totals/KPIs need semantic-model calculations.",
            "Build the final table, matrix, or chart visual.",
...
def render_drilldown_output_inspector(report_row, final_df, drilldown_df, lineage_df=None):
    ...
    st.markdown(f"**Power BI Output Recommendation:** {output_recommendation}")
    hold_outputs = []
    ...
    if drilldown_df is None or drilldown_df.empty:
        ...
        df['Power BI replacement'] = df['power_bi_replacement']
        show_table_or_info(df[['WebFOCUS drilldown', 'Target report', 'Parameter', 'Power BI replacement']], "No drilldown detected.")


def render_power_query_inspector(report_row):
    sql_needed = report_row.get('SQL View Need') == 'Recommended'
    if sql_needed:
        st.write("Power Query should be light cleanup only.")
        st.info(f"Because SQL View is recommended, Power Query should not rebuild the full WebFOCUS logic. Use Power Query only after `{sql_view_name(report_row)}` is created and validated.")
        steps = [
            f"Connect to {sql_view_name(report_row)}",
            "Confirm preview loads",
            "Set data types",
            "Rename business-facing columns",
            "Remove technical/helper columns",
            "Keep final report fields",
            "Load to model",
        ]
    else:
        st.write("Power Query should recreate the WebFOCUS staging flow.")
        steps = [
    ...
    for idx, step in enumerate(steps, start=1):
        st.checkbox(step, value=False, key=f"pq_inspector_{report_row['FEX Name']}_{idx}")
...
def infer_dax_measure_name(field_name):
...
def render_dax_inspector(report_row, field_df, formula_df):
    st.info("Do not move all WebFOCUS DEFINE formulas into DAX. Most row-level formulas should be handled in SQL View or Power Query.")
    candidates = []
    ...
    if not candidates:
        st.info("No obvious DAX measures detected. Add DAX only for totals, KPIs, or interactive aggregations.")
        return
    ...
    for field in list(dict.fromkeys(candidates))[:8]:
        measure_name = infer_dax_measure_name(field)
        st.code(f"{measure_name} =\nSUM ( {final_table}[{field}] )", language='DAX')


def render_data_model_and_dax_inspector(report_row, source_df, mapping_df, field_df, formula_df):
    st.subheader("Data Model")
    render_data_model_inspector(report_row, source_df, mapping_df)
    st.subheader("DAX")
    render_dax_inspector(report_row, field_df, formula_df)
...
def render_visuals_inspector(report_row, final_df, field_df):
    ...
    if 'GRAPH' in output_text:
        ...
        visual_type = 'Matrix visual'
    else:
        visual_type = 'Table visual'

    st.markdown(f"**Recommended Power BI Page:** {Path(str(report_row.get('FEX Name', 'report'))).stem.replace('_', ' ').title()}")
    ...
    st.write("Slicers: Date fields, customer fields, product fields, order/status fields, and WebFOCUS parameters.")
    st.warning("Review: confirm which numeric fields should be summed before creating final DAX.")
...
def render_detailed_report_summary(report_row, source_df, lineage_df, join_df, filter_df, formula_df):
    ...
    if sql_need == 'Recommended':
        summary += (
            ...
            "a SQL view that reproduces the WebFOCUS data preparation logic. Power BI should connect to the SQL view "
            "and use Power Query only for light cleanup such as data types, naming, and final field selection. "
        )
    else:
        summary += (
            "Based on the detected logic, this can likely be migrated with Power Query source shaping and a Power BI report visual. "
        )
...
def render_walkthrough(lineage_df):
    ...
    for _, row in lineage_df.sort_values('Step #').iterrows():
        ...
        with st.expander(step_title, expanded=int(row['Step #']) <= 3):
            ...
            st.write(f"Power BI action: {row['Power BI Action'] or 'Review this step and rebuild it in Power Query or the semantic model.'}")
            if row.get('Output Table'):
                st.write(f"Output table: {row['Output Table']} ({row['Output Role']})")
            if row.get('Power BI Table Suggestion'):
                st.write(f"Suggested Power BI table/query: {row['Power BI Table Suggestion']}")
...
def render_pipeline(lineage_df, final_df):
    ...
    for _, row in lineage_df.sort_values('Step #').iterrows():
        ...
        output_value = str(row.get('Output Table', '') or row.get('Power BI Table Suggestion', '') or '').strip()
    ...
    if not final_df.empty:
        ...
        lines.append(f"   -> {final_row.get('Suggested Power BI Visual / Output', 'Power BI visual')}")

    st.code('\n'.join(lines).strip(), language='text')


def render_power_query_checklist(build_plan_df):
    if build_plan_df.empty:
        st.info("No Power Query checklist was generated.")
        return

    pq_rows = build_plan_df[
        build_plan_df['What To Build'].astype(str).str.contains('Power Query|staging|merge|source query|custom column|unpivot', case=False, na=False)
    ]
    if pq_rows.empty:
        pq_rows = build_plan_df

    for _, row in pq_rows.sort_values('Plan Step').iterrows():
        label = f"Step {row['Plan Step']}: {row['Power BI Layer'] or row['WebFOCUS Object']}"
        with st.expander(label, expanded=int(row['Plan Step']) <= 4):
            st.checkbox(row['What To Build'], value=False, key=f"pq_{row['File Name']}_{row['File Path']}_{row['Plan Step']}")
            st.write(row['Why'])
...
def render_mapping_cards(mapping_df):
    ...
    for table_name, group in mapping_df.groupby('Mapping Table Name'):
        with st.expander(f"{table_name} ({len(group)} rows detected)", expanded=False):
            ...
            st.write(first['Suggested Power BI Action'])
            cols = [
                'Mapping Table Name',
                'Source Field',
                'Source Value',
...
def render_visual_plan(final_df):
    ...
    for _, row in final_df.iterrows():
        with st.container(border=True):
            ...
            st.markdown(f"**Visual/output:** {row['Suggested Power BI Visual / Output']}")
...
def collect_review_items(build_plan_df, joins_df, filters_df, formulas_df):
    ...
    if not build_plan_df.empty:
        for _, row in build_plan_df[build_plan_df['Manual Review'].astype(str).eq('Yes')].iterrows():
            items.append(('Medium', row['Power BI Layer'] or row['WebFOCUS Object'], row['Why'], row['What To Build']))
    ...
    if not joins_df.empty:
        for _, row in joins_df.iterrows():
            ...
            items.append((priority, 'Join key check', row.get('Review Reason', 'Confirm join behavior.'), row.get('Suggested Power BI Action', 'Review join.')))
    ...
    if not filters_df.empty:
        ...
        for _, row in risky_filters.iterrows():
            items.append(('Medium', 'Parameter/filter check', row['Expression'], row['Power BI Guidance']))
    ...
    if not formulas_df.empty:
        ...
        for _, row in risky_formulas.iterrows():
            ...
            items.append((priority, f"Formula: {row['Field Name']}", row['Raw WebFOCUS Formula'], row['Suggested Power BI Expression / Action'] or 'Review formula rebuild.'))
...
def render_sql_view_plan(build_plan_df, report_row):
    ...
    if build_plan_df is not None and not build_plan_df.empty:
        sql_rows = build_plan_df[
            build_plan_df['Power BI Layer'].astype(str).str.contains('SQL View', case=False, na=False)
        ]

    if sql_rows.empty and report_row.get('SQL View Need') == 'Not Needed':
        st.success("SQL View Need: Not Needed")
        st.write("This report appears suitable for Power Query and the Power BI visual layer.")
        return
    ...
    st.markdown(
        ...
        <div class="wizard-card">
          ...
          <p><strong>Action:</strong> {report_row.get('Power BI Action', 'Review SQL View need.')}</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    if sql_rows.empty:
        ...
        for _, row in sql_rows.iterrows():
            with st.expander(f"Step {row['Plan Step']}: {row['Power BI Layer']}", expanded=True):
                st.write(row['What To Build'])
                st.write(row['Why'])
                st.warning("Validate row counts and join cardinality against WebFOCUS output.")


def render_dax_plan(formula_df, final_df):
    if formula_df is None or formula_df.empty:
        st.info("No DAX-specific formulas were detected. This may be mostly Power Query and visual setup.")
        return

    dax_candidates = formula_df[
        formula_df['Formula Type'].astype(str).str.contains('Arithmetic|Conditional|Date/Time|Direct/Other', case=False, na=False)
    ]
    if dax_candidates.empty:
        st.info("No clear DAX candidates were detected. Use Power Query for source shaping and mapping logic.")
        return

    st.write("Review these formulas and decide whether they belong in Power Query custom columns or DAX measures/calculated columns.")
    show_table_or_info(
        dax_candidates[[
            'Field Name',
            'Formula Type',
            'Raw WebFOCUS Formula',
            'Suggested Power BI Expression / Action',
            'Needs Manual Review',
        ]],
        "No DAX candidates found.",
    )
...
def render_validation_checklist(report_row, join_df, final_df, drilldown_df=None):
    ...
    st.caption("Use this before marking the report as rebuilt in Power BI. Work through each group, tick completed checks, and add tester notes if needed.")
    ...
    st.text_area(
        ...
        placeholder="Example: WebFOCUS row count = 12,450; Power BI row count = 12,450. Totals matched.",
        height=110,
    )
...
def render_sources_hold_combined(source_df, lineage_df, report_row):
    ...
    st.markdown('<div class="tab-intro">Confirm which files and lookup tables feed the report before rebuilding it in Power BI.</div>', unsafe_allow_html=True)
...
def render_joins_filters_combined(join_df, filter_df, report_row):
    ...
    with st.expander("Show raw filter expressions"):
        cols = [col for col in ['Filter Type', 'Expression', 'Parameter', 'Fields Used', 'Power BI Guidance'] if col in filter_df.columns]
        show_table_or_info(
...
def render_joins_tab(join_df, report_row):
    st.subheader("Joins")
    st.markdown('<div class="tab-intro">Review JOIN and MATCH logic, keys, and whether the relationship belongs in SQL, Power Query, or the Power BI model.</div>', unsafe_allow_html=True)
    render_joins_matches_inspector(join_df, report_row)
    with st.expander("Show raw JOIN/MATCH statements"):
        cols = [col for col in ['Mapping Type', 'From Table', 'From Key', 'To Table', 'To Key', 'Raw WebFOCUS Statement', 'Review Reason'] if col in join_df.columns]
...
def render_filters_tab(filter_df):
    ...
    st.markdown('<div class="tab-intro">Translate WebFOCUS WHERE, IF, and parameter logic into Power Query filters, report slicers, or DAX rules.</div>', unsafe_allow_html=True)
    render_filters_parameters_inspector(filter_df)
    with st.expander("Show raw filter expressions"):
        cols = [col for col in ['Filter Type', 'Expression', 'Parameter', 'Fields Used', 'Power BI Guidance'] if col in filter_df.columns]
...
def render_power_bi_build_plan(report_row, source_df, lineage_df, join_df, filter_df, formula_df, mapping_df, final_df, field_df, drilldown_df=None):
    st.info("The full evidence workbook is available from the Download Full Excel Report button.")

    ai_tab, sql_tab, pq_tab, model_dax_tab, visual_tab, validation_tab = st.tabs([
        "AI Build Plan",
        "SQL View",
        "Power Query",
        "Data Model & DAX",
    ...
    with pq_tab:
        render_power_query_inspector(report_row)

    with model_dax_tab:
        render_data_model_and_dax_inspector(report_row, source_df, mapping_df, field_df, formula_df)
    ...
    try:
        ...
        if overview_df.empty:
            ...
            if not selected_overview.empty:
                ...
                with report_tabs[5]:
                    render_power_bi_build_plan(report_row, source_df, lineage_df, join_df, filter_df, formula_df, mapping_df, final_df, field_df, drilldown_df)
