...
def generate_decode_calculated_field(source_field, derived_field, pairs):
    lines = [f"// Tableau calculated field: {derived_field}", "CASE [" + source_field + "]"]
    for source_value, mapped_value in pairs:
        safe_source = source_value.replace('"', '\\"')
        safe_mapped = mapped_value.replace('"', '\\"')
        lines.append(f'    WHEN "{safe_source}" THEN "{safe_mapped}"')
...
def generate_decode_data_source_join_step(source_field, derived_field, lookup_table_name):
    return (
        f"-- Tableau Data Source setup for {lookup_table_name}\n"
        f"1. Connect {lookup_table_name} as an additional data source/table.\n"
        f"2. In the Data Source tab, create a relationship or join:\n"
        f"   Main table.{source_field} = {lookup_table_name}.{source_field}\n"
        f"3. Drag {lookup_table_name}.{derived_field} into use as a dimension in worksheets.\n"
        f"   (Alternative: build a calculated field with CASE/IF logic instead of a join\n"
        f"   if the lookup is small and static.)"
    )


def build_decode_migration_pack(mapping_df, final_table_name, switch_threshold=60):
    packs = []
    ...
    for table_name, group in mapping_df.groupby('Mapping Table Name'):
        ...
        if len(pairs) <= switch_threshold:
            packs.append({
                'mapping_table_name': table_name,
                'source_field': source_field,
                'derived_field': derived_field,
                'pair_count': len(pairs),
                'recommended_approach': 'Tableau calculated field (CASE)',
                'code': generate_decode_calculated_field(source_field, derived_field, pairs),
            })
        else:
            ...
            packs.append({
                ...
                'derived_field': derived_field,
                'pair_count': len(pairs),
                'recommended_approach': f'Tableau lookup data source ({lookup_name})',
                'lookup_csv': generate_decode_lookup_table_csv(source_field, derived_field, pairs),
                'merge_code': generate_decode_data_source_join_step(source_field, derived_field, lookup_name),
            })
            ...
            if count >= min_branches:
                results.append({
                    ...
                    'suggestion': (
                        ...
                        'or directly to a Tableau lookup data source.'
                    ),
                })
    return results
...
def build_rule_findings(parsed, mapping_df, final_table_name):
    ...
    if date_idiom:
        findings.append({
            ...
            'recommendation': 'Convert positional date-string parsing into a typed Date field in the Tableau data source (Data Prep step or a DATE() calculated field).',
...
def blank_llm_translation(status='Not requested'):
    return {
        'suggested_tableau_type': '',
        'suggested_tableau_expression_action': '',
        'llm_notes': '',
...
def translate_expression_with_llm(client, model, expression_type, expression, context):
    ...
    prompt = f"""
You translate WebFOCUS migration logic into Tableau guidance.
cd 
...
Rules:
- The value for suggested_tableau_expression_action must be one plain multiline string.
- Do not return suggested_tableau_expression_action as a JSON array, Python list, bullet list, or numbered list.
...
- Prefer a Tableau calculated field for row-level DEFINE/COMPUTE logic.
- Prefer a table calculation only when the aggregation/window context is clearly needed (e.g. running total, rank).
- Prefer a Data Prep (Tableau Prep) step or data source join/union for source shaping, filters before load, and cleanup.
- For JOIN or MATCH, explain the likely data source relationship, join, or union approach.
- For filters and joins, the suggested output may be an action, not a formula.
- Keep data source and field names as placeholders when the exact Tableau workbook name is unknown.
...
""".strip()
    ...
    try:
        ...
        result = {
            'suggested_tableau_type': normalize_llm_text(data.get('suggested_tableau_type', '')),
            'suggested_tableau_expression_action': normalize_llm_text(data.get('suggested_tableau_expression_action', '')),
            ...
            'review_reason': normalize_llm_text(data.get('review_reason', '')) or 'Review before using in Tableau.',
...
def _numeric_measure_candidates(field_df):
    """Reuses the same value-field heuristic as the rule-based calculated-field
...
AI_BUILD_PLAN_SYSTEM_PROMPT = """You are a senior BI migration engineer converting one parsed WebFOCUS report to Tableau.
...
Tableau target concepts to use:
- Data Source Query: initial SQL/table connection.
- Data Prep Step: Tableau Prep flow step (cleaning, pivot, join, union, calculation before load).
- Mapping Data Source: a small lookup table joined/related to the main data source.
- Published Data Source: a shared, reusable Tableau data source.
- Calculated Field (Measure) / Calculated Field (Dimension): row-level or aggregate derived fields
  (equivalent to WebFOCUS DEFINE/COMPUTE).
- Filter: a worksheet/dashboard filter (equivalent to WebFOCUS WHERE/IF).
- Parameter: equivalent to a WebFOCUS parameter (&VARIABLE), used for user-driven inputs.
- Worksheet: a single Tableau view/chart.
- Dashboard: a page combining multiple worksheets.
- Dashboard Action: filter/URL/highlight actions (equivalent to a WebFOCUS drilldown/URL link).
...
Return JSON with exactly these keys:
{
  ...
  "implementation_plan": [{"step_number": 1, "title": "", "action": "",
                            "tableau_layer": "Data Source Query|Data Prep Step|Mapping Data Source|Published Data Source|Calculated Field (Measure)|Calculated Field (Dimension)|Filter|Parameter|Worksheet|Dashboard Action|Dashboard|Manual Review",
...
VALID_TABLEAU_LAYERS = {
    "Data Source Query", "Data Prep Step", "Mapping Data Source", "Published Data Source",
    "Calculated Field (Measure)", "Calculated Field (Dimension)", "Filter", "Parameter",
    "Worksheet", "Dashboard Action", "Dashboard", "Manual Review",
}

_LAYER_PRECEDENCE = [
    "Data Prep Step", "Data Source Query", "Mapping Data Source", "Published Data Source",
    "Calculated Field (Measure)", "Calculated Field (Dimension)", "Filter", "Parameter",
    "Worksheet", "Dashboard Action", "Dashboard", "Manual Review",
]

def _coerce_tableau_layer(value):
    if value in VALID_TABLEAU_LAYERS:
        return value
    parts = re.split(r'[|/,]', str(value))
    parts = [p.strip() for p in parts if p.strip() in VALID_TABLEAU_LAYERS]
    if parts:
...
def normalize_tableau_layers(raw):
    section_keys = [
    ...
    for key in section_keys:
        ...
        if not isinstance(steps, list):
            continue
        for step in steps:
            if isinstance(step, dict) and 'tableau_layer' in step:
                step['tableau_layer'] = _coerce_tableau_layer(step['tableau_layer'])
    return raw
...
def generate_ai_build_plan(client, model, payload):
    ...
    while True:
        ...
        raw = normalize_tableau_layers(raw)
...
def _render_plan_step(step, source_df):
    ...
    with st.container(border=True):
        st.markdown(f"**{step.title}** — {step.tableau_layer}")
        st.markdown(
...
def build_tableau_prep_build_order(lineage_df, plan):
    """Combines the HOLD chain's real execution order (from lineage_df) with
    the AI plan's Data Prep steps, so the report maker gets one numbered
    sequence matching the order they'd build steps in Tableau Prep or the
    Data Source tab - instead of hunting through separate Data Prep/
    Calculation cards."""
    ordered_stages = []
    ...
    if plan is not None:
        prep_steps = [
            ...
            if step.tableau_layer in {"Data Prep Step", "Mapping Data Source", "Data Source Query"}
        ]
...
def render_tableau_prep_build_order(lineage_df, plan):
    stages = build_tableau_prep_build_order(lineage_df, plan)
    if not stages:
        st.info("No staging steps detected for this report - it likely reads directly from source with no HOLD chain.")
        return

    st.markdown(
        "Build these as **Tableau Prep flow steps (or Data Source tab steps), in this exact order**. "
...
def render_ai_build_plan(report_row, source_df, lineage_df, join_df, filter_df, formula_df, mapping_df, final_df, field_df):
    st.markdown(
        ...
        "calls - join risk, novel formulas, and worksheet/dashboard suggestions for **Tableau**."
    )
    ...
    for item in decode_pack:
        with st.container(border=True):
            ...
            if 'lookup_csv' in item:
                ...
                st.caption("Tableau data source setup:")
    ...
    with tabs[0]:
        render_tableau_prep_build_order(lineage_df, plan)
...
def raw_db_fields(formula, defined_names):
    tokens = re.findall(r'\b([A-Z][A-Z0-9_]{2,}|[0-9][A-Z][A-Z0-9_]{1,})\b', formula.upper())

    return sorted({
        t for t in tokens
        if t not in WF_KEYWORDS and t not in defined_names and not t.isdigit()
    })
...
def suggest_formula_rebuild(formula_type):
    if 'Decode/Mapping' in formula_type:
        return 'Use a Tableau CASE calculated field, or a joined lookup data source.'
    if 'Conditional IF' in formula_type:
        return 'Use a Tableau IF/CASE calculated field.'
    if 'Concatenation/String' in formula_type:
        return 'Use Tableau string functions (SPLIT, REPLACE, string concatenation with +) in a calculated field.'
    if 'Arithmetic' in formula_type:
        return 'Use a Tableau calculated field or a Data Prep custom calculation step.'
    if 'Parameter-based' in formula_type:
        return 'Use a Tableau parameter and filter/calculated field driven by it.'
    if 'Date/Time' in formula_type:
        return 'Use Tableau date functions (DATEPART, DATEADD, DATEDIFF) in a calculated field or Data Prep step.'
    return 'Review expression and rebuild as a Tableau calculated field or Data Prep step.'

def suggest_filter_rebuild(expr):
    if extract_parameters(expr):
        return 'Use a Tableau parameter with a filter or calculated field driven by it.'
    return 'Use a worksheet/dashboard filter, a context filter, or a Data Prep row filter.'

def suggest_output_format(fmt):
    fmt = str(fmt or '').upper()
    if fmt in {'HTML', 'HTMTABLE', 'HTMLCSS'}:
        return 'Tableau dashboard published to Tableau Server/Cloud, or embedded view.'
    if fmt in {'PDF'}:
        return 'Tableau dashboard exported/scheduled as PDF.'
    if fmt in {'XLSX', 'EXL2K', 'EXCEL', 'CSV', 'ALPHA', 'DFIX'}:
        return 'Tableau download as crosstab/CSV/Excel, or a scheduled data export.'
    if fmt in {'FOCUS', 'HOLD'}:
        return 'Tableau Prep output step or an extracted data source.'
    return 'Map to a Tableau worksheet, dashboard, or export based on usage.'


def suggest_join_tableau_action(from_table, from_key, join_mode, to_table, to_key):
    ...
    action = (
        f"Create a Data Source relationship:\n{from_ref} -> {to_ref}\n\n"
        f"Alternative: use a physical join in the Data Source tab between {from_table or 'FROM_TABLE'} "
        f"and {to_table or 'TO_TABLE'} on {from_key or 'FROM_KEY'} = {to_key or 'TO_KEY'} if fields from "
        f"{to_table or 'TO_TABLE'} must be blended row-by-row before analysis."
    )
    if str(join_mode or '').upper() == 'JOIN TO ALL':
        action += (
            "\n\nWebFOCUS TO ALL suggests multi-row matching may exist. Review key uniqueness, "
            "cardinality, and duplicate row expansion before implementing as a Tableau join."
        )
    return action
...
def suggest_join_review_reason(join_mode):
    if str(join_mode or '').upper() == 'JOIN TO ALL':
        return 'JOIN TO ALL can imply one-to-many or many-to-one behavior. Confirm key uniqueness, cardinality, and row duplication in Tableau.'
    return 'Confirm whether this should be a Tableau data source relationship or a physical join, and verify key uniqueness/cardinality.'


def suggest_match_tableau_action(first_table, to_tables, keys, hold):
    ...
    return (
        ...
        f"{hold_text}\n\n"
        "Rebuild as a Tableau Prep flow step (join or union), or an extracted staging data source. "
        "Review OLD/NEW/AFTER/MORE behavior manually before choosing join vs. union."
    )
...
def extract_filter_parameter_mapping(text, defined_names):
    ...
    for match in filter_pattern.finditer(text):
        ...
        rows.append({
            ...
            'tableau_guidance': suggest_filter_rebuild(expr),
        })
    ...
    for param in extract_parameters(text):
        ...
        parameter_rows.append({
            ...
            'parameters': [param],
            'fields': [],
            'tableau_guidance': 'Create a Tableau parameter and drive a filter or calculated field from it.',
        })
    ...
    for command, pattern in patterns:
        for match in pattern.finditer(text):
            ...
            rows.append({
                ...
                'format': fmt,
                'tableau_guidance': suggest_output_format(fmt),
            })
...
def extract_join_match_mapping(text):
    ...
    for match in join_pattern.finditer(text):
        ...
        rows.append({
            ...
            'tableau_guidance': 'Use a data source relationship or physical join; TO ALL usually maps to one-to-many/many-to-one behavior.',
            'suggested_tableau_type': 'Data source relationship / physical join',
            'suggested_tableau_expression_action': suggest_join_tableau_action(
                from_table, from_key, join_type, to_table, to_key,
            ),
    ...
    for match in match_block_pattern.finditer(text):
        ...
        rows.append({
            ...
            'tableau_guidance': 'Use a Tableau Prep union/join or a data source relationship depending on MATCH logic and AFTER/MORE usage.',
            'suggested_tableau_type': 'Tableau Prep staging flow / union',
            'suggested_tableau_expression_action': suggest_match_tableau_action(
                first_table, ', '.join(dict.fromkeys(all_tables[1:])), ', '.join(keys), hold,
            ),
            'needs_manual_review': 'Yes',
    ...
    for pattern in patterns:
        for match in pattern.finditer(text):
            ...
            rows.append({
                ...
                'tableau_replacement': 'Tableau dashboard action (filter, URL, or highlight action) or a navigation button',
...
def extract_source_inventory(parsed):
    ...
    for source in sources:
        ...
        if source_type in {'CSV', 'Excel', 'Flat file', 'File'}:
            action = f"Connect {layer} as a Tableau data source from the FILEDEF path."
        elif source_type == 'Database / synonym':
            action = f"Connect {layer} as a Tableau database/synonym data source connection."
        else:
            action = f"Create a staging data source/Data Prep flow {layer}; confirm the synonym's physical source."
        ...
        rows.append({
            ...
            'Suggested Tableau Layer': layer,
            'Suggested Tableau Action': action,
...
def extract_hold_lineage(text, parsed):
    ...
    for join in parsed.get('join_match_mapping', []):
        ...
        rows.append({
            ...
            'Formulas Created': '',
            'Filters Applied': '',
            'Tableau Table Suggestion': '',
            'Tableau Action': join.get('suggested_tableau_expression_action', ''),
    ...
    for match in extract_table_blocks(clean_text):
        ...
        tableau_table = suggested_stage_name(table_prefix, output or source)
        action_parts = []
        if output_role in {'Intermediate HOLD', 'Default HOLD'}:
            action_parts.append('Create this as a Tableau Prep flow step / staging data source.')
        else:
            action_parts.append('Use this as the final published data source or worksheet dataset.')
        ...
        if filters:
            action_parts.append('Apply row filters before load or as report/page filters.')
        ...
        rows.append({
            ...
            'Tableau Table Suggestion': tableau_table,
            'Tableau Action': ' '.join(action_parts),
    ...
    for match_row in parsed.get('join_match_mapping', []):
        ...
        rows.append({
            ...
            'Formulas Created': '',
            'Filters Applied': '',
            'Tableau Table Suggestion': suggested_stage_name('stg', ...),
            'Tableau Action': match_row.get('suggested_tableau_expression_action', ''),
    ...
    for item in parsed.get('formula_mapping', []):
        ...
        for source_value, mapped_value in pairs:
            rows.append({
                ...
                'Suggested Tableau Action': f"Create {table_name} as a Tableau lookup data source and join it on {source_field}.",
            })
...
def recommend_final_dataset(parsed):
    ...
    if has_across or len([m for m in measures.split(', ') if m]) > 1:
        ...
        reason = 'WebFOCUS uses multiple measures or ACROSS-style categories. Tableau stacked/multi-measure charts usually work best when category/type is stored as rows (long format).'
    else:
    ...
    return [{
        'Recommended Final Dataset': last_step.get('Tableau Table Suggestion') or 'final_ReportDataset',
        'Dataset Shape': shape,
        'Reason': reason,
        'Suggested Tableau Visual / Output': 'Tableau stacked/multi-measure chart' if has_graph else output_type_text(parsed) or 'Tableau worksheet/crosstab',
...
def analyze_sql_view_need(parsed):
    ...
    if source_count >= 3 or match_count or join_to_all_count or join_count >= 2 or hold_count >= 3 or score >= 8:
        need = 'Recommended'
        reason = 'Complex JOIN/HOLD/MATCH logic is easier to validate and reuse as a SQL view before connecting Tableau.'
        path = 'SQL View -> Tableau Data Source / Prep -> Calculated Fields -> Worksheet'
        action = 'Create a SQL view for joins/HOLD logic, then connect Tableau and finish shaping/reporting.'
    elif join_count == 1 or hold_count in {1, 2} or complex_formula_count >= 2:
        need = 'Optional'
        reason = 'The report has moderate shaping logic. Tableau Prep/Data Source can handle it, but a SQL view may be cleaner if reused.'
        path = 'Tableau Data Source / Prep -> Calculated Fields -> Worksheet'
        action = 'Use Tableau Prep/Data Source first; consider a SQL view if performance, reuse, or governance is important.'
    else:
        need = 'Not Needed'
        reason = 'The report appears straightforward enough for Tableau Data Source/Prep and the worksheet layer.'
        path = 'Tableau Data Source / Prep -> Worksheet'
        action = 'Load source, apply filters/calculations in Tableau, then build the worksheet/dashboard.'
    if 'GRAPH' in output_formats and 'Worksheet' not in path:
        path = f"{path} -> Worksheet"
...
def build_tableau_plan(parsed):
    ...
    if sql_view['need'] in {'Recommended', 'Optional'}:
        rows.append({
            'Plan Step': step, 'Tableau Layer': 'SQL View',
            'WebFOCUS Object': 'JOIN/HOLD/MATCH logic',
            'What To Build': sql_view['action'], 'Why': sql_view['reason'], 'Manual Review': 'Yes',
        })
        step += 1

    for source in parsed.get('source_inventory', []):
        rows.append({
            'Plan Step': step, 'Tableau Layer': source['Suggested Tableau Layer'],
            'WebFOCUS Object': source['Source Name'],
            'What To Build': source['Suggested Tableau Action'],
            'Why': 'Start with source/staging data sources before rebuilding HOLD outputs.',
    ...
    for lineage in parsed.get('hold_lineage', []):
        rows.append({
            'Plan Step': step, 'Tableau Layer': lineage['Tableau Table Suggestion'],
            'WebFOCUS Object': lineage['Output Table'] or lineage['Input Table(s)'],
            'What To Build': lineage['Tableau Action'],
            'Why': lineage['What Happens In This Step'], 'Manual Review': lineage['Needs Manual Review'],
        })
        step += 1

    for mapping in parsed.get('mapping_table_preview', []):
        rows.append({
            'Plan Step': step, 'Tableau Layer': mapping['Mapping Table Name'],
            'WebFOCUS Object': mapping['Derived Field'],
            'What To Build': mapping['Suggested Tableau Action'],
            'Why': 'DECODE logic is easier to maintain as a lookup data source.',
            'Manual Review': 'Yes',
        })
        step += 1
    ...
    for final_dataset in parsed.get('final_dataset_plan', []):
        rows.append({
            'Plan Step': step, 'Tableau Layer': final_dataset['Recommended Final Dataset'],
            'WebFOCUS Object': 'Final output',
            'What To Build': f"Build {final_dataset['Dataset Shape']} dataset for {final_dataset['Suggested Tableau Visual / Output']}.",
            'Why': final_dataset['Reason'], 'Manual Review': final_dataset['Needs Manual Review'],
        })
...
def parse_fex(fex_text):
    ...
    for f in result['define_fields']:
        ...
        result['formula_mapping'].append({
            ...
            'tableau_guidance': suggest_formula_rebuild(formula_type),
        })
    ...
    result['tableau_build_plan'] = build_tableau_plan(result)
...
def build_validation_rows_raw(
    ...
    if sql_view_need == 'Recommended':
        add_check(
            'SQL / Data Validation',
            'Validate SQL view output before connecting Tableau.',
    ...
    add_check(
        'SQL / Data Validation',
        'Compare final row count between WebFOCUS output and the Tableau data source.',
        'Always',
    )
    add_check(
...
def build_display_tables(parsed_results, duplicate_records, allowed_program_names, matched_pairs):
    ...
    for folder, fex_name, parsed in parsed_results:
        ...
        sql_view = analyze_sql_view_need(parsed) if parsed is not None else {
            ...
            'action': 'Review parser output before choosing SQL View, Tableau Prep, calculated fields, or the worksheet layer.',
            'score': 0,
        }
        complexity_score = (
        ...
        overview_rows.append({
            ...
            'Tableau Action': sql_view['action'],
            ...
            'Build Plan Steps': len(parsed['tableau_build_plan']) if parsed is not None else 0,
        ...
        for item in parsed['filter_parameter_mapping']:
            filter_rows.append({
                ...
                'Tableau Guidance': item['tableau_guidance'],
                'Suggested Tableau Type': item.get('suggested_tableau_type', ''),
                'Suggested Tableau Expression / Action': item.get('suggested_tableau_expression_action', ''),
        ...
        for item in parsed['join_match_mapping']:
            join_rows.append({
                ...
                'Suggested Tableau Action': item.get('suggested_tableau_expression_action', ''),
                'Needs Manual Review': item.get('needs_manual_review', ''),
                'Review Reason': item.get('review_reason', ''),
                'Confidence': item.get('confidence', ''),
            })
        ...
        for item in parsed['tableau_build_plan']:
            row = {'File Path': folder, 'File Name': fex_name}
            row.update(item)
...
st.markdown(
    ...
    <div class="kash-hero">
      ...
      <div class="kash-subtitle">Break Down <span class="kash-subtitle-accent">WebFOCUS</span> Complexity for Faster <span class="kash-subtitle-accent">Tableau</span> Migration</div>
    </div>
    """,
    unsafe_allow_html=True,
)
...
with st.expander("How to use KashMap", expanded=True):
    st.markdown(
        ...
        4. Review report cards first, then open the guided Tableau build plan.
...
with st.expander("Translate one WebFOCUS expression or query", expanded=False):
    ...
    if st.button("Translate to Tableau", key="manual_llm_translate"):
        client = get_openai_client()
        if client is None:
            ...
            st.subheader("Tableau Translation")
            st.write("**Suggested Tableau Type**")
            st.write(result.get("suggested_tableau_type", "") or "Not returned")
            st.write("**Suggested Tableau Expression / Action**")
            st.code(result.get("suggested_tableau_expression_action", "") or "Not returned", language="text")
            st.write("**LLM Notes**")
            ...
            st.write(result.get("review_reason", "") or "Review before using in Tableau.")
            st.write("**Confidence**")
...
def render_overview_story(report_row, source_df, final_df):
    ...
    dataset_name = final_text.get('Recommended Final Dataset', report_row.get('Recommended Final Dataset', 'final Tableau data source'))
    visual = final_text.get('Suggested Tableau Visual / Output', report_row.get('Output Format', 'Tableau worksheet/dashboard'))
    shape = final_text.get('Dataset Shape', 'Tableau-ready data source')
    ...
    with st.container(border=True):
        ...
        st.markdown(f"**Tableau version:** build **{dataset_name}** as a **{shape}**, then use it for the final worksheet or dashboard.")
        st.markdown(
            f"{badge('Build type: Tableau dashboard')} "
...
def source_role_text(source_name, sql_need):
    ...
    if sql_need == 'Optional':
        return 'Tableau Prep/Data Source or SQL view'
    return 'Load in Tableau Prep/Data Source'


def recommend_formula_layer(formula_type):
    text = str(formula_type or '')
    if 'Decode/Mapping' in text:
        return 'Lookup data source, then SQL View / Tableau join'
    if 'Date/Time' in text or 'Concatenation/String' in text or 'Conditional IF' in text:
        return 'SQL View or Tableau calculated field'
    if 'Arithmetic' in text:
        return 'SQL View / Tableau calculated field'
    if 'Parameter-based' in text:
        return 'Tableau parameter plus SQL/Data Prep filter'
    return 'Review; likely SQL View or Tableau Prep/Data Source'
...
def build_start_here_facts(report_row, source_df, lineage_df, join_df, final_df):
    ...
    recommended_layer = 'SQL View' if sql_need == 'Recommended' else 'Tableau Prep/Data Source'
    if sql_need == 'Optional':
        recommended_layer = 'SQL View or Tableau Prep/Data Source'
...
def fallback_report_summary(facts):
    ...
    if duplicate_status in {'Exact Duplicate', 'Near Duplicate', 'Same Data Source'}:
        duplicate_text = (f"{duplicate_status} - rebuild once where possible and reuse the same Tableau approach for this group.")
    else:
        duplicate_text = "Unique - no duplicate group was detected for this report."

    if facts['recommended_layer'] == 'SQL View':
        direction = ("Build the preparation logic in a SQL view first, then connect Tableau and validate row counts/totals.")
    elif facts['recommended_layer'] == 'SQL View or Tableau Prep/Data Source':
        direction = ("Use a SQL view or Tableau Prep/Data Source for the preparation logic, then connect Tableau and validate row counts/totals.")
    else:
        direction = ("Build the preparation logic in Tableau Prep/Data Source, then create the Tableau worksheet/dashboard and validate row counts/totals.")
...
def render_source_files_tables(source_df, report_row):
    ...
    df['Tableau Action'] = df['Source Name'].apply(lambda x: source_role_text(x, report_row.get('SQL View Need')))
    show_table_or_info(df[['Source', 'How detected', 'Type', 'Role in Report', 'Tableau Action']], "No source files/tables were detected.")
...
def render_hold_lineage_inspector(lineage_df, report_row):
    st.write("WebFOCUS HOLD files are temporary datasets. In Tableau migration, these usually become SQL CTEs, SQL view steps, or Tableau Prep staging flow steps.")
    ...
    hold_df['Migration equivalent'] = hold_df['Output Role'].apply(
        lambda role: 'SQL CTE inside SQL view' if report_row.get('SQL View Need') == 'Recommended' else 'Tableau Prep staging step'
    )
...
def render_joins_matches_inspector(join_df, report_row):
    ...
    if report_row.get('SQL View Need') == 'Recommended':
        df['Recommended Tableau/SQL Action'] = 'Implement inside SQL view first. Use a Tableau data source relationship only if the joined table remains separate.'
    else:
        df['Recommended Tableau/SQL Action'] = df['Suggested Tableau Action']
    df['Review'] = df['Review Reason']
    show_table_or_info(df[['Risk', 'Join / Match', 'Left Side', 'Right Side', 'Recommended Tableau/SQL Action', 'Review']], "No JOIN/MATCH statements were detected.")
...
def render_filters_parameters_inspector(filter_df):
    ...
    df['Tableau Treatment'] = df.apply(short_filter_treatment, axis=1)
    field_col = 'Fields Used' if 'Fields Used' in df.columns else 'Fields'
    show_table_or_info(df[['Filter / Parameter', field_col, 'Meaning', 'Tableau Treatment']], "No filters or parameters were detected.")
...
def short_filter_treatment(row):
    ...
    if filter_type == 'PARAMETER' or parameter.strip():
        return 'Tableau parameter'
    if filter_type == 'WHERE':
        return 'Fixed SQL filter' if 'SQL' in str(row.get('Tableau Guidance', '')).upper() else 'Tableau Prep row filter'
    if filter_type == 'IF':
        return 'SQL WHERE / Tableau Prep row filter'
    if '&' in expression:
        return 'Tableau parameter'
...
def sql_view_reasons(report_row, source_df, lineage_df, join_df):
    ...
    if report_row.get('SQL View Need') == 'Not Needed':
        reasons.append("Simple enough for Tableau Prep/Data Source based on current parser rules")
    return reasons
...
def build_sql_review_todos(source_df, lineage_df, join_df, filter_df, formula_df):
    todos = [
        'Verify physical database schema/table names before running this SQL.',
        'Replace SELECT * with the exact report fields needed.',
        'Validate row counts and totals against WebFOCUS output.',
    ]
    if join_df is not None and not join_df.empty:
        todos.append('Verify join type, join key formatting, null behavior, duplicates, and cardinality.')
        if (join_df['Mapping Type'].astype(str) == 'MATCH FILE').any():
            todos.append('MATCH FILE detected: WebFOCUS MATCH behavior may not equal a normal SQL JOIN. Validate OLD/NEW/AFTER/MORE behavior.')
        if (join_df['WebFOCUS Join Mode'].astype(str).str.upper() == 'JOIN TO ALL').any():
            todos.append('JOIN TO ALL detected: confirm one-to-many behavior and whether duplicate expansion is expected.')
    if filter_df is not None and not filter_df.empty:
        todos.append('Convert WebFOCUS WHERE/IF filters to SQL WHERE clauses only after confirming parameter behavior.')
    if formula_df is not None and not formula_df.empty:
        formula_types = ' '.join(formula_df['Formula Type'].dropna().astype(str).tolist())
        if 'Decode/Mapping' in formula_types:
            todos.append('DECODE mappings detected: consider mapping tables or reviewed SQL CASE expressions.')
        if any(token in formula_types for token in ['Date/Time', 'Parameter-based']):
            todos.append('Date/time or parameter formulas detected: review SQL dialect-specific conversion logic.')
    return todos


def generate_sql_view_draft(report_row, source_df, lineage_df, join_df, filter_df=None, formula_df=None):
    ...
    for source in sources:
        ctes.append(
            f"""-- Source table detected from WebFOCUS
{sql_source_cte_name(source)} AS (
    SELECT
        *
    FROM {source}
)"""
    ...
    previous_cte = sql_source_cte_name(sources[0]) if sources else 'source_table'
    hold_rows = []
    if lineage_df is not None and not lineage_df.empty and 'Output Table' in lineage_df:
        hold_rows = [
            row for _, row in lineage_df.sort_values('Step #').iterrows()
            if str(row.get('Output Table', '')).strip()
        ]

    for row in hold_rows:
        hold_name = str(row.get('Output Table', '')).strip()
        cte_name = sql_identifier(hold_name, f"hold_{row.get('Step #', 'step')}")
        input_name = str(row.get('Input Table(s)', '')).split(',')[0].strip()
        from_cte = sql_identifier(input_name, '') or previous_cte
        if input_name in sources:
            from_cte = sql_source_cte_name(input_name)

        ctes.append(
            f"""-- WebFOCUS HOLD equivalent: {hold_name}
{cte_name} AS (
    SELECT
        -- TODO: replace * with fields detected for {hold_name}
        *
    FROM {from_cte}
    -- TODO: apply formulas, aggregations, BY/ACROSS logic, and filters for {hold_name}
)"""
        )
    ...
    if join_df is not None and not join_df.empty:
        for _, row in join_df.iterrows():
            join_comment_lines.append(
                f"    -- TODO: {row.get('Mapping Type', 'JOIN')} {row.get('From Table', '')}.{row.get('From Key', '')} "
                f"to {row.get('To Table', '')}.{row.get('To Key', '')}; verify join type/cardinality"
            )
            if row.get('Mapping Type') == 'MATCH FILE':
                join_comment_lines.append("    -- REVIEW: MATCH FILE may require UNION/FULL OUTER JOIN/staged logic depending on OLD/NEW/AFTER/MORE.")

    filter_comment_lines = []
    if filter_df is not None and not filter_df.empty:
        for _, row in filter_df.head(12).iterrows():
            if row.get('Filter Type') in {'WHERE', 'IF'}:
                filter_comment_lines.append(f"    -- TODO filter: {row.get('Expression', '')}")

    formula_comment_lines = []
    if formula_df is not None and not formula_df.empty:
        for _, row in formula_df.head(12).iterrows():
            formula_comment_lines.append(f"        -- TODO formula {row.get('Field Name', '')}: {row.get('Raw WebFOCUS Formula', '')}")

    final_cte = sql_identifier(report_row.get('Recommended Final Dataset') or f"final_{Path(str(report_row.get('FEX Name', 'report'))).stem}", 'final_dataset')
    final_source = previous_cte
    if not ctes:
        ctes.append(
            """-- TODO: no source CTEs were detected
source_table AS (
    SELECT *
    FROM TODO_SOURCE_TABLE
)"""
        )
        final_source = 'source_table'

    ctes.append(
        f"""-- Final report dataset blueprint
{final_cte} AS (
    SELECT
{chr(10).join(formula_comment_lines) if formula_comment_lines else '        *'}
    FROM {final_source}
{chr(10).join(join_comment_lines) if join_comment_lines else '    -- TODO: add JOIN/MATCH logic if required'}
{chr(10).join(filter_comment_lines) if filter_comment_lines else '    -- TODO: apply fixed filters if required'}
)"""
    )

    todo_lines = '\n'.join(f"-- REVIEW: {todo}" for todo in build_sql_review_todos(source_df, lineage_df, join_df, filter_df, formula_df))
    cte_sql = ',\n\n'.join(ctes)
    return f"""-- Rule-based SQL View Draft / SQL View Blueprint
-- Generated from parsed WebFOCUS structure.
-- This is a starting point, not production-ready SQL.
{todo_lines}
...
def render_sql_view_recommendation_inspector(report_row, source_df, lineage_df, join_df, filter_df=None, formula_df=None):
    ...
    st.write("**Preferred:** create this as a database view, then connect Tableau's data source directly to the view.")
    st.write("**Alternative:** use Tableau's custom SQL connection with the final reviewed SELECT query if database permissions do not allow creating views.")

    st.subheader("SQL CTE Outline")
    st.code(build_sql_cte_outline(source_df, lineage_df, join_df, report_row), language='text')

    st.subheader("Rule-based SQL View Draft")
    st.markdown(
        """
        <div class="wizard-card">
          <h3>SQL Draft Status</h3>
          <p><strong>Generated by:</strong> Rule-based parser</p>
          <p><strong>Ready to run:</strong> No — review required</p>
          <p><strong>Use as:</strong> Starting blueprint</p>
          <p><strong>Must review:</strong> schema names, SELECT fields, JOIN types, MATCH FILE behavior, filters, and row counts</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
    st.caption(
        "This SQL is generated from parsed WebFOCUS structure. It is intended as a starting point. "
        "A developer must verify schemas, data types, formulas, MATCH FILE behavior, joins, and row counts before production use."
    )
    sql_draft = generate_sql_view_draft(report_row, source_df, lineage_df, join_df, filter_df, formula_df)
    st.code(sql_draft, language='sql')
    st.download_button(
        "Download .sql",
        data=sql_draft.encode('utf-8'),
        file_name=f"{view_name}.sql",
        mime="text/sql",
        key=f"download_sql_{report_row.get('FEX Name', 'report')}_{report_row.get('File Path', '')}",
    )

    with st.expander("Show SQL review warnings only"):
        for todo in build_sql_review_todos(source_df, lineage_df, join_df, filter_df, formula_df):
            st.write(f"- {todo}")
...
def render_tableau_approach_inspector(report_row, build_plan_df):
    need = report_row.get('SQL View Need', 'Needs Review')
    view_name = f"vw_{Path(str(report_row.get('FEX Name', 'report'))).stem.lower()}_base"
    if need == 'Recommended':
        st.write("**Recommended approach:** SQL View -> Tableau Data Source/Prep -> Dashboard")
        steps = [
            f"Create SQL View: ask the DBA/migration developer to create `{view_name}`.",
            f"Connect Tableau Desktop to `{view_name}`.",
            "Clean in Data Source/Prep: set data types, rename fields, remove unused columns, and keep report-needed fields.",
            "Build the worksheet/dashboard: create the final crosstab, table, or chart.",
            "Validate: compare row counts, totals, filters, and join results against WebFOCUS output.",
        ]
    else:
        st.write("**Recommended approach:** Tableau Data Source/Prep -> Dashboard")
        steps = [
            "Connect Tableau to the source file/table.",
            "Use Tableau Prep/Data Source for filters, type cleanup, and report-specific transformations.",
            "Create calculated fields only if final totals/KPIs need row-level or aggregate logic.",
            "Build the final crosstab, table, or chart.",
...
def render_drilldown_output_inspector(report_row, final_df, drilldown_df, lineage_df=None):
    ...
    output_recommendation = 'Crosstab / List Report'

    if 'GRAPH' in str(report_row.get('Output Format', '')).upper():
        output_recommendation = 'Chart'

    st.markdown(f"**Final Dataset Detected:** `{final_dataset}`")
    st.markdown(f"**Tableau Output Recommendation:** {output_recommendation}")
    hold_outputs = []
    ...
    if drilldown_df is None or drilldown_df.empty:
        ...
        df['Tableau replacement'] = df['tableau_replacement']
        show_table_or_info(df[['WebFOCUS drilldown', 'Target report', 'Parameter', 'Tableau replacement']], "No drilldown detected.")


def render_data_prep_inspector(report_row):
    sql_needed = report_row.get('SQL View Need') == 'Recommended'
    if sql_needed:
        st.write("Tableau Prep/Data Source should be light cleanup only.")
        st.info(f"Because a SQL View is recommended, Tableau Prep should not rebuild the full WebFOCUS logic. Use it only after `{sql_view_name(report_row)}` is created and validated.")
        steps = [
            f"Connect Tableau data source to {sql_view_name(report_row)}", "Confirm preview loads", "Set data types",
            "Rename business-facing fields", "Remove technical/helper columns", "Keep final report fields",
            "Publish or embed the data source",
        ]
    else:
        st.write("Tableau Prep should recreate the WebFOCUS staging flow.")
        steps = [
            "Load source files/tables", "Create staging Prep flow steps", "Apply row filters",
            "Create calculated fields", "Join/union source tables", "Create final output data source",
        ]
    for idx, step in enumerate(steps, start=1):
        st.checkbox(step, value=False, key=f"prep_inspector_{report_row['FEX Name']}_{idx}")
...
def infer_calculated_field_name(field_name):
...
def render_calculated_field_inspector(report_row, field_df, formula_df):
    st.info("Do not move all WebFOCUS DEFINE formulas into calculated fields. Most row-level formulas should be handled in SQL View or Tableau Prep.")
    candidates = []
    ...
    if not candidates:
        st.info("No obvious summary measures detected. Add a calculated field only for totals, KPIs, or interactive aggregations.")
        return
    ...
    for field in list(dict.fromkeys(candidates))[:8]:
        measure_name = infer_calculated_field_name(field)
        st.code(f"// Tableau calculated field: {measure_name}\nSUM([{field}])", language='text')
...
def render_data_model_and_calc_inspector(report_row, source_df, mapping_df, field_df, formula_df):
    ...
    render_calculated_field_inspector(report_row, field_df, formula_df)
...
def render_visuals_inspector(report_row, final_df, field_df):
    ...
    st.markdown(f"**Recommended Tableau Dashboard:** {Path(str(report_row.get('FEX Name', 'report'))).stem.replace('_', ' ').title()}")
    ...
    st.write("Filters: Date fields, customer fields, product fields, order/status fields, and WebFOCUS parameters.")
    st.warning("Review: confirm which numeric fields should be summarized before creating final calculated fields.")
...
def render_detailed_report_summary(report_row, source_df, lineage_df, join_df, filter_df, formula_df):
    ...
    if sql_need == 'Recommended':
        summary += (
            ...
            "a SQL view that reproduces the WebFOCUS data preparation logic. Tableau should connect to the SQL view "
            "and use Tableau Prep only for light cleanup such as data types, naming, and final field selection. "
        )
    else:
        summary += (
            "Based on the detected logic, this can likely be migrated with Tableau Prep/Data Source shaping and a Tableau worksheet. "
        )
...
def render_walkthrough(lineage_df):
    ...
    for _, row in lineage_df.sort_values('Step #').iterrows():
        ...
        with st.expander(step_title, expanded=int(row['Step #']) <= 3):
            ...
            st.write(f"Tableau action: {row['Tableau Action'] or 'Review this step and rebuild it in Tableau Prep or the data source.'}")
            if row.get('Output Table'):
                st.write(f"Output table: {row['Output Table']} ({row['Output Role']})")
            if row.get('Tableau Table Suggestion'):
                st.write(f"Suggested Tableau table/data source: {row['Tableau Table Suggestion']}")
...
def render_pipeline(lineage_df, final_df):
    ...
    for _, row in lineage_df.sort_values('Step #').iterrows():
        ...
        output_value = str(row.get('Output Table', '') or row.get('Tableau Table Suggestion', '') or '').strip()
    ...
    if not final_df.empty:
        ...
        lines.append(f"   -> {final_row.get('Suggested Tableau Visual / Output', 'Tableau visual')}")

    st.code('\n'.join(lines).strip(), language='text')


def render_tableau_prep_checklist(build_plan_df):
    if build_plan_df.empty:
        st.info("No Tableau Prep checklist was generated.")
        return
    prep_rows = build_plan_df[
        build_plan_df['What To Build'].astype(str).str.contains('Tableau Prep|staging|join|source query|calculated field|unpivot', case=False, na=False)
    ]
    if prep_rows.empty:
        prep_rows = build_plan_df
    for _, row in prep_rows.sort_values('Plan Step').iterrows():
        label = f"Step {row['Plan Step']}: {row['Tableau Layer'] or row['WebFOCUS Object']}"
        with st.expander(label, expanded=int(row['Plan Step']) <= 4):
            st.checkbox(row['What To Build'], value=False, key=f"prep_{row['File Name']}_{row['File Path']}_{row['Plan Step']}")
            st.write(row['Why'])
...
def render_mapping_cards(mapping_df):
    ...
    for table_name, group in mapping_df.groupby('Mapping Table Name'):
        with st.expander(f"{table_name} ({len(group)} rows detected)", expanded=False):
            ...
            st.write(first['Suggested Tableau Action'])
            cols = [
                'Mapping Table Name',
                'Source Field',
                'Source Value',
...
def render_visual_plan(final_df):
    ...
    for _, row in final_df.iterrows():
        with st.container(border=True):
            ...
            st.markdown(f"**Visual/output:** {row['Suggested Tableau Visual / Output']}")
...
def collect_review_items(build_plan_df, joins_df, filters_df, formulas_df):
    ...
    if not build_plan_df.empty:
        for _, row in build_plan_df[build_plan_df['Manual Review'].astype(str).eq('Yes')].iterrows():
            items.append(('Medium', row['Tableau Layer'] or row['WebFOCUS Object'], row['Why'], row['What To Build']))
    ...
    if not joins_df.empty:
        for _, row in joins_df.iterrows():
            ...
            items.append((priority, 'Join key check', row.get('Review Reason', 'Confirm join behavior.'), row.get('Suggested Tableau Action', 'Review join.')))
    ...
    if not filters_df.empty:
        ...
        for _, row in risky_filters.iterrows():
            items.append(('Medium', 'Parameter/filter check', row['Expression'], row['Tableau Guidance']))
    ...
    if not formulas_df.empty:
        ...
        for _, row in risky_formulas.iterrows():
            ...
            items.append((priority, f"Formula: {row['Field Name']}", row['Raw WebFOCUS Formula'], row['Suggested Tableau Expression / Action'] or 'Review formula rebuild.'))
...
def render_sql_view_plan(build_plan_df, report_row):
    ...
    if build_plan_df is not None and not build_plan_df.empty:
       sql_rows = build_plan_df[build_plan_df['Tableau Layer'].astype(str).str.contains('SQL View', case=False, na=False)]

    if sql_rows.empty and report_row.get('SQL View Need') == 'Not Needed':
        st.write("This report appears suitable for Tableau Prep/Data Source and the worksheet layer.")
        return
    ...
    st.markdown(
        ...
        <div class="wizard-card">
          ...
          <p><strong>Action:</strong> {report_row.get('Tableau Action', 'Review SQL View need.')}</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    if sql_rows.empty:
        ...
        for _, row in sql_rows.iterrows():
            with st.expander(f"Step {row['Plan Step']}: {row['Tableau Layer']}", expanded=True):
                st.write(row['What To Build'])
                st.write(row['Why'])
                st.warning("Validate row counts and join cardinality against WebFOCUS output.")


def render_calc_field_plan(formula_df, final_df):
    if formula_df is None or formula_df.empty:
        st.info("No calculated-field-specific formulas were detected. This may be mostly Tableau Prep and worksheet setup.")
        return
    ...
    if calc_candidates.empty:
        st.info("No clear calculated field candidates were detected. Use Tableau Prep for source shaping and mapping logic.")
        return
    st.write("Review these formulas and decide whether they belong in Tableau Prep custom steps or worksheet calculated fields.")
    show_table_or_info(
        calc_candidates[['Field Name', 'Formula Type', 'Raw WebFOCUS Formula', 'Suggested Tableau Expression / Action', 'Needs Manual Review']],
        "No calculated field candidates found.",
    )
...
def render_validation_checklist(report_row, join_df, final_df, drilldown_df=None):
    ...
    st.caption("Use this before marking the report as rebuilt in Tableau. Work through each group, tick completed checks, and add tester notes if needed.")
    ...
    st.text_area(
        ...
        placeholder="Example: WebFOCUS row count = 12,450; Tableau row count = 12,450. Totals matched.",
        height=110,
    )
...
def render_sources_hold_combined(source_df, lineage_df, report_row):
    ...
    st.markdown('<div class="tab-intro">Confirm which files and lookup tables feed the report before rebuilding it in Tableau.</div>', unsafe_allow_html=True)
...
def render_joins_filters_combined(join_df, filter_df, report_row):
    ...
    with st.expander("Show raw filter expressions"):
        cols = [col for col in ['Filter Type', 'Expression', 'Parameter', 'Fields Used', 'Tableau Guidance'] if col in filter_df.columns]
        show_table_or_info(
...
def render_joins_tab(join_df, report_row):
    st.subheader("Joins")
    st.markdown('<div class="tab-intro">Review JOIN and MATCH logic, keys, and whether the relationship belongs in SQL, Tableau Prep, or the Tableau data source.</div>', unsafe_allow_html=True)
    render_joins_matches_inspector(join_df, report_row)
    with st.expander("Show raw JOIN/MATCH statements"):
        cols = [col for col in ['Mapping Type', 'From Table', 'From Key', 'To Table', 'To Key', 'Raw WebFOCUS Statement', 'Review Reason'] if col in join_df.columns]
...
def render_filters_tab(filter_df):
    ...
    st.markdown('<div class="tab-intro">Translate WebFOCUS WHERE, IF, and parameter logic into Tableau Prep filters, dashboard filters, or calculated field rules.</div>', unsafe_allow_html=True)
    render_filters_parameters_inspector(filter_df)
    with st.expander("Show raw filter expressions"):
        cols = [col for col in ['Filter Type', 'Expression', 'Parameter', 'Fields Used', 'Tableau Guidance'] if col in filter_df.columns]
...
def render_tableau_build_plan(report_row, source_df, lineage_df, join_df, filter_df, formula_df, mapping_df, final_df, field_df, drilldown_df=None):
    ...
    ai_tab, sql_tab, prep_tab, model_calc_tab, visual_tab, validation_tab = st.tabs([
        "AI Build Plan", "SQL View", "Tableau Prep / Data Source", "Data Model & Calculated Fields", "Visuals", "Validation",
    ])
    with ai_tab:
    ...
    try:
        ...
        if overview_df.empty:
            ...
            if not selected_overview.empty:
                ...
                with report_tabs[5]:
                    render_tableau_build_plan(report_row, source_df, lineage_df, join_df, filter_df, formula_df, mapping_df, final_df, field_df, drilldown_df)
